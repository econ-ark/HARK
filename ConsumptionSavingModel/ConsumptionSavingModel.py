...
class SetupImperfectForesightSolver(PerfectForesightSolver):
    ...
    def setAndUpdateValues(self,solution_tp1,income_distrib,survival_prob,beta):
        ...
        if self.cubic_splines or self.calc_vFunc:
            ...
            self.kappa_max_t = 1.0/(1.0 + (self.p_worst_income**(1/self.rho))* \
    ...
    def defineBorrowingConstraint(self,constraint):
        ...
        natural_constraint = (self.solution_tp1.m_underbar - self.xi_underbar_tp1)*(self.Gamma*self.psi_underbar_tp1)/self.R
        self.m_underbar_t = np.max([natural_constraint,constraint])
        if natural_constraint < self.m_underbar_t: 
            self.kappa_max_t = 1.0 # If actually constrained, MPC near limit is 1
...
class ConsumptionSavingSolverMarkov(ConsumptionSavingSolverENDG):
    ...
    def __init__(self,solution_tp1,income_distrib_list,p_zero_income_array,survival_prob,beta,
                      rho,R,Gamma,transition_array,constraint,a_grid,calc_vFunc,cubic_splines):
    ...
    def solve(self):
        ...
        if self.calc_vFunc or self.cubic_splines:
            exp_kappa_max_tp1 = (np.dot(self.transition_array,self.p_zero_income_array*self.solution_tp1.kappa_max**(-self.rho))/
...
def consumptionSavingSolverMarkov(solution_tp1,income_distrib,p_zero_income,survival_prob,
                                      beta,rho,R,Gamma,transition_array,constraint,a_grid,calc_vFunc,cubic_splines):
                                       
    solver = ConsumptionSavingSolverMarkov(solution_tp1,income_distrib,p_zero_income,
                                               survival_prob,beta,rho,R,Gamma,transition_array,constraint,a_grid,
                                               calc_vFunc,cubic_splines)
    #solver.prepareToSolve()              
    solution                   = solver.solve()
