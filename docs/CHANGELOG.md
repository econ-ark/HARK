...
- Wires `HARK.dual_measure`'s base-draw cache through `IndShockConsumerType.get_shocks` and `MarkovConsumerType.get_shocks`: under the opt-in `_cache_base_shock_draws` flag, income-shock uniforms are recorded for Q-CDF inversion using the same draws and the same inversion as the default path — the P-stream is bit-identical with the flag on or off (tested). [#1799](https://github.com/econ-ark/HARK/pull/1799)
