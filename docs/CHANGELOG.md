...
- Adds `HARK.tail_interpolation`: representations for buffer-stock consumption functions above the top gridpoint. `PowerLawDecayLinearInterp`/`PowerLawDecayCubicHermiteInterp` replace the stock exponential-gap extrapolation with the theoretically correct power-law decay toward the perfect-foresight bound (level+slope matched at the top knot by default; explicit exponent override supported); `retrofit_powerlaw` upgrades already-constructed stock interpolants in place (interior coefficients preserved bit-for-bit); `MoMLogGapChartInterp`/`chartify_in_place` re-chart a solved consumption function in Method-of-Moderation coordinates (log gap vs log wealth, hermite or shape-preserving pchip slopes), whose linear continuation IS a power-law attach with the locally measured exponent; `measure_local_q` estimates that exponent from a solved policy's own top knots (two-secant log-gap slopes + a convergence-drift diagnostic), closing the loop so the module is usable end-to-end without an externally supplied Q. Supersedes the interpolation-classes portion of draft #1782. [#1818](https://github.com/econ-ark/HARK/pull/1818)
