...
- Fixes value functions under log utility (`CRRA = 1`), which most consumption-saving models either failed to build or built wrong ([#75](https://github.com/econ-ark/HARK/issues/75)). With log utility, value is additive in permanent income, `V(M, P) = v(m) + log(P) / MPCmin`, so continuation value gains a `log(PermGroFac * PermShk)` term where other CRRAs multiply by `(PermGroFac * PermShk)**(1 - CRRA)`, and the pseudo-inverse value `exp(v)` curves like `m**(1 / MPCmin)`. `ValueFuncCRRA` takes a new `vScale`, so solvers store `exp(v / vScale)`, which is close to linear in `m`. The perfect foresight, IndShock, KinkedR, PrefShock, KinkyPref, RiskyAsset, Portfolio, SequentialPortfolio, RiskyContrib, warm-glow bequest, GenIncProcess, wealth-in-utility, WealthPortfolio, CapitalistSpirit, MedExtMarg and Markov solvers now build it, and `PerfForesightConsumerTypeFast` and `IndShockConsumerTypeFast` accept `CRRA = 1`. `MedShockConsumerType` raises when asked for a value function at `CRRA = 1`, since its value reaches `-6e6` and `exp(v)` underflows at any scale. `ShareLimit`, which raised at `CRRA = 1`, is now computed there. `ValueFuncCRRA` defaults `vScale` to 1 at the class level, so instances pickled before it existed still evaluate. Apart from the fixes in the next entry, solutions at every other CRRA are bit-identical. `StoneGearyCRRAutility_invP`, which no solver calls, changes at every CRRA: its leading factor is now the `1 / factor` its derivative needs, where it was `1 / (1 - rho)`.
- Fixes three value-function bugs that apply at every CRRA. `RiskyContribConsumerType` with a continuous contribution share and `AdjustPrb < 1` raised `AttributeError` when building its value function. In the warm-glow portfolio terminal solution, `cFuncFxd` raised `TypeError` and `vFuncFxd` returned consumption. With `AdjustPrb < 1` that wrong value entered continuation value and moved discrete share choices. `WealthPortfolioConsumerType` left wealth out of flow utility when building its value function, so at default parameters value missed the Bellman equation by 0.22.
