...
- Fixes `MarkovConsumerType`'s value function ignoring survival: the solver scaled end-of-period marginal value by `LivPrb` for the current state but added end-of-period value unscaled, so `vFunc` was the value of an agent who never dies whenever `LivPrb < 1`. Consumption was unaffected, since it depends only on marginal value.
...
- Fixes the income-weighted (Harmenberg) transition matrices under mortality with permanent-income growth, in the model-file Simulator (`AgentSimulator.make_transition_matrices`, and so `find_steady_state`, `simulate_shock_by_grids` and `make_basic_SSJ`) and in `NewKeynesianConsumerType.calc_transition_matrix`. Both reweighted survivors by the permanent shock alone and re-injected newborns at the population death rate; with `LivPrb < 1` and `PermGroFac != 1` that is not the income-weighted measure (the young are overweighted at every age), so long-run averages and Jacobians of normalized variables were biased -- at the `IndShockConsumerType` defaults, which the SSJ tutorial uses, consumption 0.5 % and assets 3 % low against Monte Carlo, and an order of magnitude more for very patient households. The weight is now the realized growth of the normalized level (`norm='G'` in the model files, i.e. `PermGroFac * PermShk`) relative to the growth newborns inherit (`newborn_growth`, defaulting to the agent's `PermGroFacAgg`), newborns enter at the complement of the surviving mass, and outcome reads are normalized by the mass growth within the period; `PermGroFacAgg = PermGroFac` reproduces the previous construction as the trend convention (newborns inherit all growth), and `norm='PermShk'` is unchanged (exact only without growth or without mortality). `NewKeynesianConsumerType.calc_transition_matrix` also omitted `PermGroFac` from the normalized transition itself (`m' = R a / (G psi) + theta`); fixed. Both tools now agree on the same primitives to 1e-5 on consumption. Results change only when survival is below one and growth differs from one; the SSJ notebooks now pass `norm='G'`. The life-cycle Jacobian helper `make_flat_LC_SSJ_matrices` (reached through `make_basic_SSJ` when `cycles=1`) is brought under the same weighting: each birth cohort's income-weighted mass is propagated age by age with the per-state surviving mass, so the deterministic growth is carried by the masses and a shock that changes the income process moves them (the shocked masses enter the news term); `newborn_growth` is passed through (defaulting to one, the flat-demographics case the function implements); every period is built without the population-level replacement a cohort model does not need; and the Jacobians are expressed per unit of the newborn cohort's first-period level. `norm='G'` with `trend=None` and `norm='PermShk'` with `trend='PermGroFac'` give the same Jacobians for shocks that leave the income process alone, and the latter is unchanged from before.
- Fixes the income-weighted (Harmenberg) transition matrices under mortality with permanent-income growth, in the model-file Simulator (`AgentSimulator.make_transition_matrices`, and so `find_steady_state`, `simulate_shock_by_grids` and `make_basic_SSJ`) and in `NewKeynesianConsumerType.calc_transition_matrix`. Both reweighted survivors by the permanent shock alone and re-injected newborns at the population death rate; with `LivPrb < 1` and `PermGroFac != 1` that is not the income-weighted measure (the young are overweighted at every age), so long-run averages and Jacobians of normalized variables were biased -- at the `IndShockConsumerType` defaults, which the SSJ tutorial uses, consumption 0.5 % and assets 3 % low against Monte Carlo, and an order of magnitude more for very patient households. The weight is now the realized growth of the normalized level (`norm='G'` in the model files, i.e. `PermGroFac * PermShk`) relative to the growth newborns inherit (`newborn_growth`, defaulting to the agent's `PermGroFacAgg`), newborns enter at the complement of the surviving mass, and outcome reads are normalized by the mass growth within the period; `PermGroFacAgg = PermGroFac` reproduces the previous construction as the trend convention (newborns inherit all growth), and `norm='PermShk'` is unchanged (exact only without growth or without mortality). `NewKeynesianConsumerType.calc_transition_matrix` also omitted `PermGroFac` from the normalized transition itself (`m' = R a / (G psi) + theta`); fixed. Both tools now agree on the same primitives to 1e-5 on consumption. Results change only when survival is below one and growth differs from one; the SSJ notebooks now pass `norm='G'`.
- Adds a `ghost` option to `make_basic_SSJ` and `calc_impulse_response_manually` (default `False`, behavior unchanged): when set, the perturbed finite-horizon chain is differenced against an *unperturbed* finite-horizon chain of the same length solved from the same steady state -- a ghost run -- instead of against the steady-state solution itself. The two differ by the residual of the steady-state solve, which the backward chain keeps converging away from: at HARK's default `tolerance` (1e-6) that residual is negligible for an impatient household but for a patient one (growth-patience factor near one) it is divided by `eps` and shows up as spurious mass in the long-horizon columns -- 70 % of the largest entry on a household near the growth-impatience bound, where the ghost-differenced Jacobian matches the one from a steady state converged to 1e-10 to 0.2 %. Costs one extra finite-horizon solve and matrix build per call; the ghost run's drift from the steady state is reported under `verbose`. The life-cycle builder is unaffected (its terminal condition is exact).
- Fixes `calc_impulse_response_manually` under `offset=True`: it dropped the first simulated period from its output but let the finite-horizon agent's period-0 policy respond, so the shock was announced one period before its reported date 0 while `make_basic_SSJ` announces at date 0 -- the two disagreed by 61 % at `s = 0` (a factor of six for a constructed shock), decaying to 1e-5 by `s = 50`, where the existing test compared them. The dropped period now carries the long run policy. Also fixes a `TypeError` for scalar shock parameters (e.g. `DiscFac`) in the same function, and adds tests at `s = 0, 1, 2`. Closes [#1825](https://github.com/econ-ark/HARK/issues/1825).
- Under `norm` with mortality, `make_basic_SSJ` and `calc_impulse_response_manually` now carry the response of the total income mass (`income_mass=True`, default): a shock to survival, to permanent-income growth, or to transitions among states that differ in either changes the survivors' income-mass growth, which moves the level of the normalizing variable -- for a perpetual-youth model until the affected cohorts are replaced -- while the transition matrices, which keep each row stochastic by injecting newborns at the row's deficit, assumed the mass constant. The fake-news algorithm runs on the augmented state (distribution, normalized income mass) with the recurrence linearized: the news carries an income-mass term, the distribution news is renormalized rather than topped up with newborns, and the expectation vectors carry one extra scalar; `return_mass_response=True` also returns the income-mass Jacobian, which for a one-state model has the closed form `LivPrb * (LivPrb * PermGroFac)^k` after a growth shock. Shocks that leave the income-mass growth unchanged (interest factor, discount factor, transitory income) are unaffected to machine precision; `income_mass=False` reproduces the previous construction.
...
- `setup_Q_measure` no longer emits one degenerate-distribution warning per period; a stock `init_lifecycle` produced 25, which hid the aggregate warning that matters. The periods are recorded in `Q_degenerate_periods` instead. A Markov period counts when every state's distribution is degenerate. No Markov period was recorded before, because each period's newly built list of Q distributions was compared with its P list by identity.
...
- `DualMeasureMixin` runs its Q step from an `_sim_period_epilogue` override, where `t_age` and `t_cycle` still hold the values the P step used, and no longer overrides `sim_one_period`. A change to the base pipeline therefore reaches dual mode with no copy to update. Verified bit-identical. Dual-mode `simulate` raises on the first period of a subclass whose own `sim_one_period` never calls `_sim_period_epilogue`, which would otherwise run with a `history_Q` full of NaN.
- `PermanentIncomeNormalizationMixin` caches each age's cumulative log-pLvl moments for the run. It recomputed them from birth for every age present in every period, so the cost of normalizing grew with the cube of the simulation's length. Simulated output is unchanged.
