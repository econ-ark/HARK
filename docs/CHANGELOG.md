...
- Adds two options to `AgentType` for infinite-horizon solves. `stopping_rule='contraction'` stops on the distance to the fixed point that the per-cycle contraction rate implies (the last step times r / (1 - r), r floored over the last three ratios) rather than on the last step itself; near the growth-impatience edge r is close to one and the last step understates the remaining distance by 40 to 160 times, so the default rule ('step', unchanged) stops with consumption 1e-4 to 1e-3 from the fixed point at the default tolerance while the new rule brings it under 1e-6 for 1.05 to 1.5 times the cycles. `report_convergence=True` attaches a report to the agent and to `solution[0]` (and prints it under verbose): the last step, the contraction rate and the implied distance, and for `IndShockConsumerType` the two target levels of market resources (E[m'|m] = m, and the mortality-adjusted level at which the population's mean is stationary under perpetual youth), the knots spanning each, the last step and the implied distance there, and a flag when a target lies above the grid; the targets come from a sign test along the knots (compare (m_i - E[theta]) / Rbar with a_i, Rbar the growth-corrected return, mortality-adjusted for the population target), exact between knots because the consumption function is linear there, at a cost of microseconds. Measured on the default and two quarterly calibrations the largest step always sits at the top of the grid, where the ergodic mass is nil, while the knots at the target are converged orders of magnitude tighter; the report makes that visible. Not available for state-dependent solutions.
#### Release Notes
