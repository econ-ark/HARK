...
- `DualMeasureMixin` runs its Q step from an `_sim_period_epilogue` override, where `t_age` and `t_cycle` still hold the values the P step used, and no longer overrides `sim_one_period`. A change to the base pipeline therefore reaches dual mode with no copy to update. Verified bit-identical.
