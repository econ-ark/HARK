...
- Fixes the income-weighted (Harmenberg) transition matrices under mortality with permanent-income growth, in the model-file Simulator (`AgentSimulator.make_transition_matrices`, and so `find_steady_state`, `simulate_shock_by_grids` and `make_basic_SSJ`) and in `NewKeynesianConsumerType.calc_transition_matrix`. Both reweighted survivors by the permanent shock alone and re-injected newborns at the population death rate; with `LivPrb < 1` and `PermGroFac != 1` that is not the income-weighted measure (the young are overweighted at every age), so long-run averages and Jacobians of normalized variables were biased -- at the `IndShockConsumerType` defaults, which the SSJ tutorial uses, consumption 0.5 % and assets 3 % low against Monte Carlo, and an order of magnitude more for very patient households. The weight is now the realized growth of the normalized level (`norm='G'` in the model files, i.e. `PermGroFac * PermShk`) relative to the growth newborns inherit (`newborn_growth`, defaulting to the agent's `PermGroFacAgg`), newborns enter at the complement of the surviving mass, and outcome reads are normalized by the mass growth within the period; `PermGroFacAgg = PermGroFac` reproduces the previous construction as the trend convention (newborns inherit all growth), and `norm='PermShk'` is unchanged (exact only without growth or without mortality). `NewKeynesianConsumerType.calc_transition_matrix` also omitted `PermGroFac` from the normalized transition itself (`m' = R a / (G psi) + theta`); fixed. Both tools now agree on the same primitives to 1e-5 on consumption. Results change only when survival is below one and growth differs from one; the SSJ notebooks now pass `norm='G'`.
- `NewKeynesianConsumerType.calc_transition_matrix` now injects newborns at their first period's income draw -- market resources equal to the transitory shock and permanent income `PermGroFac * PermShk`, where HARK's simulators place them -- instead of at `m = 1, p = 1` ("no assets and permanent income one", which skipped the newborn's first shocks). Small: on the class defaults the stationary aggregates move in the fourth significant figure (the two stored test values are updated); it removes the remaining difference from the model-file simulator's stationary distribution on the same primitives.
