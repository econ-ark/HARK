...
- Adds `HARK.stationary`, a plug-in structure for solving infinite-horizon problems directly for their stationary solution: a model poses its solution as the fixed point of a map on a vector (`StationaryProblem`), a registered method finds it (`iterate`, `anderson`; more with `register_stationary_method`), and `solve_stationary_problem` runs the method with an admissibility check and a fallback to plain iteration. `IndShockConsumerType` and `MarkovConsumerType` gain the parameters `stationary_method` (`"iterate"`, the default backward induction, or `"anderson"`) and `stationary_options`; with `"anderson"`, `solve()` on a `cycles=0` problem applies the model's own per-period solver to consumption at the asset nodes and accelerates the iteration by Anderson mixing (`HARK.utilities.anderson_accelerate`, also new), reaching the fixed point backward induction converges to in far fewer cycles when the growth impatience condition is nearly binding: about 65 against about 1,950 on a quarterly calibration with a limiting MPC near one percent, at a tolerance of 1e-10. Cubic interpolation and the value function are covered for both models. `solve_agent` dispatches through a new `AgentType.solve_stationary` hook when `stationary_method` is not `"iterate"`; the default path is unchanged.
