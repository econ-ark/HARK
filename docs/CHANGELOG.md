...
- New example notebook `examples/SequenceSpaceJacobians/TM_or_MC_which_to_use_when.ipynb`: when to push the distribution forward by transition matrices and when by Monte Carlo. Transition matrices for anything that is a property of the distribution (steady-state moments, estimation by matching moments, aggregate responses and Jacobians): deterministic, smooth in the parameters, fast. Monte Carlo for anything that depends on individual histories (realized paths, the distribution of gains across households, statistics that condition on a household's own experience), with common random numbers. Four short demonstrations on the default consumer, including an estimation objective that is smooth by one method and jagged with spurious local minima by the other, and a welfare-by-wealth-quintile calculation that only paired Monte Carlo histories can produce; the practice of HAFiscal (Carroll, Crawley, Frankovic and Tretvoll, 2026) as the worked case. Companion to `Transition_Matrix_Example.ipynb`, which covers the mechanics. Supersedes the example-notebook part of [#1750](https://github.com/econ-ark/HARK/pull/1750). [#1843](https://github.com/econ-ark/HARK/pull/1843)
