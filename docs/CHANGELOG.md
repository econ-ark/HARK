...
- Fixes `GenIncProcessConsumerType` and its subclasses (`IndShockExplicitPermIncConsumerType`, `PersistentShockConsumerType`, `CapitalistSpiritConsumerType`, `MedShockConsumerType`, `MedExtMargConsumerType`) applying expected permanent income growth twice in simulation. They inherited `IndShockConsumerType.get_shocks`, which folds `PermGroFac` into `PermShk`, while their `transition` multiplies `pLvlNextFunc`, which already carries growth, by that `PermShk`. Simulated `pLvl` grew at `PermGroFac**2` and drifted off the `pLvlGrid` the policies were solved on. `get_shocks` now takes the growth to fold in from `get_PermShk_growth_factor(t)`, which this family sets to 1.0, so `PermShk` is the pure draw the solver integrates over. `IndShockConsumerType` is unchanged. The model-file simulator already applied growth once. Only calibrations with `PermGroFac != 1` change, which includes `CapitalistSpiritConsumerType`'s default (`PermGroFac = [1.01]`). `ShockNormalizationMixin` takes its `PermShk` target from the same hook, so composing it with this family no longer rescales the pure shock up to `PermGroFac`. [#1838](https://github.com/econ-ark/HARK/issues/1838)
