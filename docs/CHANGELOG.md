...
- Adds two utilities beside `make_basic_SSJ` in `HARK.SSJutils`. `check_flow_budget` (with `flow_budget_residuals`) tests sequence space Jacobians against the household flow-budget identity column by column: consumption plus end-of-period assets, less the survivors' return on last period's assets (`Rfree * LivPrb / newborn_growth` per unit of the normalizing level), less labor income, equals what the perturbation injects, which is nothing for a perturbation of income or preferences and the assets brought into the period for one of the return factor; a failure names the date and the column. It is the acceptance test for any construction of these Jacobians and, appealing to no other construction, it also settles their dating (an interest-factor perturbation dated `s` pays on row `s` under `offset=True`). `aggregate_SSJs` adds Jacobians over types with the weights' meaning stated. Illustrated in `SSJ-advanced-examples.ipynb` with four types aggregated.
