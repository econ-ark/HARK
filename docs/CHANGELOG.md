...
- `KS-HARK-presentation.ipynb` ends with a Takeaways section: what the sequence-space method answers (the first-order response of aggregates to aggregate shocks, for any model written as blocks) and what it gives up (nonlinearity in the aggregates: large shocks, the effects of aggregate uncertainty itself, regime changes, for which the state-space `KrusellSmithType` remains the tool), and its speed on the notebook's own timings against HARK's classic Krusell-Smith example, with the model-difference caveat. `SSJ_explanation.ipynb` says the same under its advantages and links the classic example. Carries the two editorial points of [#1505](https://github.com/econ-ark/HARK/pull/1505) that main did not already have. [#1844](https://github.com/econ-ark/HARK/pull/1844)
