...
- **Breaking (Jacobian tools with a trend):** the agent's `PermGroFacAgg` is the switch between two economies, and the sequence space Jacobian tools now follow it instead of guessing. With `PermGroFacAgg == 1` every cohort is born at the same level and the economy's size is stationary (timeless questions); with any other value each newborn cohort is born that factor richer, so every level grows with calendar time and depends on the date a simulation starts (secular questions), and the age composition of the aggregate differs: about ten percent of the cumulative consumption response on a ten-year truncation of `init_lifecycle`, whose calibration carries a 1.6 percent yearly trend. `make_basic_SSJ` and `calc_impulse_response_manually` refuse to describe an economy the agent does not simulate: `newborn_growth` must imply `PermGroFacAgg`, an agent with a trend must be told which economy is meant (`newborn_growth=agent.PermGroFacAgg`, or set `PermGroFacAgg = 1.0` first), and `norm='PermShk'` without a trend factor, which treats the deterministic growth as a trend that newborns inherit, is refused unless that is the agent's choice (use `norm='G'`). The life-cycle helper applies the newborn trend on its unweighted trend path as a discount by age. A population-style run of the model-file simulator (`symulate` with the dead replaced by newborns) refuses an agent with a trend rather than silently simulating the stationary economy; cohort-style runs place no later cohorts and proceed. The simulator's blocks expose the mass behind each arrival state (`surviving_mass`, `outcome_mass`), read by the replacement of decedents and the life-cycle Jacobians instead of the layout of the `dead` matrix and the row sums of the first outcome array. The discussion is in the module docstring of `HARK.SSJutils`, "Two economies: stationary and secular". Follow-up to [#1822](https://github.com/econ-ark/HARK/pull/1822). [#1842](https://github.com/econ-ark/HARK/pull/1842)
