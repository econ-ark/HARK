...
class FashionSolution(Solution):
    ...
    def __init__(
        self, VfuncJock=None, VfuncPunk=None, switchFuncJock=None, switchFuncPunk=None
    ):
        """
...
class FashionVictimType(AgentType):
    ...
    _solution_terminal = FashionSolution(
        VfuncJock=LinearInterp(np.array([0.0, 1.0]), np.array([0.0, 0.0])),
    ...
    def updateEvolution(self):
        ...
        for j in range(self.pCount):
            ...
            dist = Uniform(
                bot=pNextMean - self.pNextWidth, top=pNextMean + self.pNextWidth
            )
    ...
    def update(self):
        ...
        None
        """
        self.conformUtilityFunc = lambda x: stats.beta.pdf(
            x, self.uParamA, self.uParamB
        )
...
def main():
    ...
    t_end = time()
    print(
        "Solving a fashion victim micro model took "
        + mystr(t_end - t_start)
        + " seconds."
    )
