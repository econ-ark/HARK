...
# $$
#
# Now assume that every consumer faces some uncertainty on her income (e.g. it follows AR (1) process, so that the current value of $Y$ is a state variable that predicts future values of $Y$), which is subject to idiosyncratic shocks - the realizations of each shock is (potentially) different for each agent. In this setting the Bellman equation looks like:
#
# \begin{eqnarray*}
