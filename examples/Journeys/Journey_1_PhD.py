...
#
# This journey does not require any special skill in programing. However, we recommend you take a few introductory tutorials in Python and object-oriented programing (OOP) to make you familiar with the basic concepts. Moreover, we assume some knowledge of economic theory.
#
# As you have found this journey, you probably have a concept of what a heterogeneous agent model is, but here is a short recap. Think about a basic infinitely lived consumer problem from first-year graduate courses, in which a consumer with a CRRA felicity function
#
# $$
# U(C)=\frac{C^{1-\rho}}{1-\rho}
# $$
#
# aims to maximize the discounted sum of felicity over a finite horizon.  Assume that every consumer faces some uncertainty on her income, which is idiosyncratic - the realization of each shock is independent of the realizations of any other shock for any other agent. In this setting the Bellman equation looks like:
#
# \begin{eqnarray*}
# V_{t} &=& \max_{C_t} U(C_t) + \mathbb{E}_{t}[\beta V_{t+1}], \\
# & s.t. & \\
# A_t &=& M_t - C_t, \\
# M_{t+1} &=& (M_{t}-C_{t})R  + Y_{t+1}, \\
# \end{eqnarray*}
#
# Therefore, finding a distribution of agent assets involves more advanced numerical tools than in the case of a representative agent. Obviously, this is more demanding to master. Moreover, the knowledge about involved numerical methods is less systematic, and often hard to find. As was mentioned in the HARK manual:
#
...
# This journey will help you mostly with using HARK in the first way. We do not elaborate here the numerical methods, however in the last sections you can find some guidance about how the source code is structured.      
#
...
# %% [markdown]
# ## 3 A few words about HARK's structure
#
...
# %% [markdown]
# ## 4 `AgentType` class 
# `AgentType` class enables you to build the macroeconomic models, such as presented in the introduction. It is also the essential part of the macroeconomic model in HARK. Therefore, to use HARK, you always need to use agent-type classes!
#
...
# \log \psi_{i,t} & \sim & \mathcal{N}(-\sigma_{\psi}^{2}/2,\sigma_{\psi}) \Rightarrow \mathbb{E}_{t}[\psi_{i,t+1}]=1\\
# \log \theta_{i,t} & \sim & \mathcal{N}(-\sigma_{\theta}^{2}/2,\sigma_{\theta})\Rightarrow \mathbb{E}_{t}[\theta_{i,t+1}]=1\\\\
# \end{eqnarray*}
#
# To get a universal solution of this problem we need to find a policy function (in this case consumption function), we can easily use the HARK `solve` function. But first we need to construct our model (we assume standard parametrization: R= 1.03, $\rho = 2$, $\beta = 0.96$, $P(\varepsilon=0)= 0.005$, $P(\varepsilon)= 0.995$, $\sigma_{\psi}= \sigma_{\theta}=0.1)$:
#
...
# \log \Psi_{t}  &\sim & {N}(-\sigma_{\Psi}^{2}/2,\sigma_{\Psi})\\
# \log \Theta_t  &\sim &{N}(-\sigma_{\Theta}^{2}/2,\sigma_{\Theta})\\
# \end{eqnarray*}
#
...
#
# The reason for this is that HARK.core is a core of the package, kind of a framework  for all models which can be coded in HARK. It contains the general framework of the `AgentType` classes (`AgentType` class) and for the market. The exact structure of modules in the HARK core you can find in the [manual](../HARKmanual/index.html) in section 0.2 General Purpose Tools. For the general structure of the AgentType and Market classes also look at the manual, for the sections 0.3 and 0.4 (you can skip the examples, as you already made a lot of them in the tutorials).
#
...
#
# The endogenous grid method is now widely used in the macroeconomic simulations; see the treatment in [QuantEcon](https://julia.quantecon.org/dynamic_programming/egm_policy_iter.html) or the appendix to [Kruger and Kindermann](https://www.nber.org/papers/w20601.pdf).
#
