...
(*
This file calculates the saving path over the course of a credit boom (2001-2007) followed by an abrupt reversal (2008-2012)
Changes in credit availability are all one-time changes. 
...
(* Calculate the credit expansion parameter: OneTimeChange, and save consumption and wealth path. *)
OneTimeChange = 0;
CreditBoom\[ScriptB] = Initial\[ScriptB];
While[CreditBoom\[ScriptB] > Target\[ScriptB],
Severance  = InitialSev;
OneTimeChange = OneTimeChange + 0.01;
Severance = Severance + OneTimeChange;
FindStableArm;
Do[
...
(* Now given OneTimeChange, we obtain path for patient people. *)
...
SavPathOverECreditCyclePat = ListPlot[{Transpose[{timePath,savRatePathPatient}]
...
SavPathOverECreditCycleImp = ListPlot[{Transpose[{timePath,savRatePathImpatient}]
},PlotStyle->{Red},
AxesLabel->{"Time","Saving Rate"},
Ticks->{{{2001,"2001"},{2007,"2007"}},{-0.1,0.1}},
PlotRange->{Automatic,PlotRangeVertical},
...
SavPathOverECreditCycleAll = ListLinePlot[{
...
SavPathOverECreditCycle = ShowLegend[Show[SavPathOverECreditCycleImp,SavPathOverECreditCyclePat],{
...
Export[FigsDir<>"/SavPathOverECreditCycle.pdf",SavPathOverECreditCycle];
Export[FigsDir<>"/SavPathOverECreditCyclePat.pdf",SavPathOverECreditCyclePat];
Export[FigsDir<>"/SavPathOverECreditCycleImp.pdf",SavPathOverECreditCycleImp];
