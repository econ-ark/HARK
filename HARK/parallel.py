...
def raiseImportError(moduleStr):
    def defineImportError(*args, **kwargs):
        raise ImportError(
            moduleStr
            + " could not be imported, and is required for this"
...
def parallelNelderMead(objFunc,guess,perturb=None,P=1,ftol=0.000001,xtol=0.00000001,
    ...
    if P > N - 1:
        print('Requested degree of simplex parallelization is ' + str(P) + ', but dimension of optimization problem is only ' + str(N-1) + '.')
