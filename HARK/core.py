...
class agent_solution(MetricObject):
    ...
    def __init__(self, *args,
        ...
        self.𝔼_tp1_ = self.E_tp1_  # After all choices, before EOP shocks
        self._t_𝔼_t = self._t_E_t  # Before BOP shocks or any choices
        self.𝔼_t = self.E_t  # After BOP shocks but before choices

    def define_reward():
        # Determine the current period payoff (utility? profit?)
        pass

    def def_transit(self):
    ...
    def solve_prepared_stage(self):
        # Hook to allow diversion in special cases
        if self.solve_prepared_stge_divert_hook():
            return self.soln_crnt  # hook should put alt soln_crnt on self

        # The current and future solution objects
        crnt, futr = self.soln_crnt, self.soln_futr

        #  Generic steps that permit most kinds of evolution of the problem
        crnt = self.def_transition_EOP__to__next_BOP  # like, aNrm to kNrm
        crnt = self.def_transition_chosen__to__EOP
        crnt = self.make_expectations_given_chosen(crnt)
        ...
        crnt = self.make_expectations_at_BOP._t_E(crnt)
        return crnt

    def E_over_BOP_shocks_defines(self):
        def _t_E(stge):
            # Make pre-BOP shocks
            pass
        pass

    def solve_prepared_stage_divert_hook(self):
        #  Special circs might require alt soln methods
        pass
