...
class testPrefShockConsumerType(unittest.TestCase):
    ...
    def test_solution(self):
        ...
        self.assertAlmostEqual(self.agent.PrefShkDstn[0][1][5],
                               0.69046812)

        self.assertAlmostEqual(self.agent.solution[0].cFunc(
            m,
            np.ones_like(m))[35],
            0.8123891603954809)

        self.assertAlmostEqual(
            self.agent.solution[0].cFunc.derivativeX(m,
                                                     np.ones_like(m))[35],
            0.44973706445183886)
