...
def solve_ConsLaborIntMarg(
    ...
    TranShkPrbs = TranShkDstn.prob
    TranShkVals = TranShkDstn.data.flatten()
    PermShkPrbs = PermShkDstn.prob
    PermShkVals = PermShkDstn.data.flatten()
...
class LaborIntMargConsumerType(IndShockConsumerType):
    ...
    def update_TranShkGrid(self):
        ...
        for t in range(self.T_cycle):
            TranShkGrid.append(
                self.TranShkDstn[t].data.flatten()
