...
# around our original method, which was plain dictionaries. 
# This structure allows us keep the original dictionary structures intact
# for legacy/importing/debugging purposes, but to augment those dictionaries
# with new content.  Once the "new content" is vetted, it can move to the
# original dicts.
@dataclass
...
class init_perfect_foresight_plus_make(AgentDict):
    def __init__(self, base_dict):
        ...
        base_dict.update(  # Default max periods
            {'aprox_par': {'MaxKinks': '500'}})
        base_dict.update(  # In principle, kinks exist all the way to infinity
            {'aprox_lim': {'MaxKinks': float('inf')}}) 
        # The simulation stge of the problem requires additional parameterization
        base_dict.update(  # The 'primitives' for the simulation
            {'prmtv_sim': ['aNrmInitMean', 'aNrmInitStd', 'pLvlInitMean', 'pLvlInitStd']})
        base_dict.update({  # Approximation parameters for monte carlo sims
            'sim_mcrlo': ['AgentCount', 'T_age']
        })
        base_dict.update({  # Limiting values that define 'true' simulation
            'sim_mcrlo_lim': {
                'AgentCount': 'infinity',
                'T_age': 'infinity'
            }
        ...
        base_dict['prmtv_par'].append('CRRA')
        # base_dict['_fcts'].update({'CRRA': CRRA_fcts})
        base_dict.update({'CRRA_fcts': CRRA_fcts})
        ...
        base_dict['prmtv_par'].append('DiscFac')
        # base_dict['_fcts'].update({'DiscFac': DiscFac_fcts})
        base_dict.update({'DiscFac_fcts': DiscFac_fcts})
        ...
        base_dict['prmtv_par'].append('LivPrb')
        # base_dict['_fcts'].update({'LivPrb': LivPrb_fcts})
        base_dict.update({'LivPrb_fcts': LivPrb_fcts})
        ...
        base_dict['prmtv_par'].append('Rfree')
        # base_dict['_fcts'].update({'Rfree': Rfree_fcts})
        base_dict.update({'Rfree_fcts': Rfree_fcts})
        ...
        base_dict['prmtv_par'].append('PermGroFac')
        # base_dict['_fcts'].update({'PermGroFac': PermGroFac_fcts})
        base_dict.update({'PermGroFac_fcts': PermGroFac_fcts})
        ...
        base_dict['prmtv_par'].append('PermGroFacAgg')
        # base_dict['_fcts'].update({'PermGroFacAgg': PermGroFacAgg_fcts})
        base_dict.update({'PermGroFacAgg_fcts': PermGroFacAgg_fcts})
        ...
        base_dict['prmtv_par'].append('BoroCnstArt')
        # base_dict['_fcts'].update({'BoroCnstArt': BoroCnstArt_fcts})
        base_dict.update({'BoroCnstArt_fcts': BoroCnstArt_fcts})
        ...
        # base_dict['prmtv_par'].append('MaxKinks')
        # base_dict['_fcts'].update({'MaxKinks': MaxKinks_fcts})
        base_dict.update({'MaxKinks_fcts': MaxKinks_fcts})
        ...
        # base_dict['sim_mcrlo'].append('AgentCount')
        # base_dict['_fcts'].update({'AgentCount': AgentCount_fcts})
        base_dict.update({'AgentCount_fcts': AgentCount_fcts})
        ...
        base_dict['sim_mcrlo'].append('aNrmInitMean')
        # base_dict['_fcts'].update({'aNrmInitMean': aNrmInitMean_fcts})
        base_dict.update({'aNrmInitMean_fcts': aNrmInitMean_fcts})

        aNrmInitStd_fcts = {
            'about': 'Std dev of initial population value of aNrm'}
        aNrmInitStd_fcts.update({'sim_mcrlo': 'True'})
        base_dict['sim_mcrlo'].append('aNrmInitStd')
        # base_dict['_fcts'].update({'aNrmInitStd': aNrmInitStd_fcts})
        base_dict.update({'aNrmInitStd_fcts': aNrmInitStd_fcts})

        pLvlInitMean_fcts = {
            'about': 'Mean initial population value of log pLvl'}
        pLvlInitMean_fcts.update({'sim_mcrlo': 'True'})
        base_dict['sim_mcrlo'].append('pLvlInitMean')
        # base_dict['_fcts'].update({'pLvlInitMean': pLvlInitMean_fcts})
        base_dict.update({'pLvlInitMean_fcts': pLvlInitMean_fcts})

        pLvlInitStd_fcts = {
            'about': 'Mean initial std dev of log ppLvl'}
        pLvlInitStd_fcts.update({'sim_mcrlo': 'True'})
        base_dict['sim_mcrlo'].append('pLvlInitStd')
        # base_dict['_fcts'].update({'pLvlInitStd': pLvlInitStd_fcts})
        base_dict.update({'pLvlInitStd_fcts': pLvlInitStd_fcts})

        T_age_fcts = {
            'about': 'Age after which simulated agents are automatically killedl'}
        T_age_fcts.update({'sim_mcrlo': 'False'})
        # base_dict['_fcts'].update({'T_age': T_age_fcts})
        base_dict.update({'T_age_fcts': T_age_fcts})

        T_cycle_fcts = {
            'about': 'Number of periods in a "cycle" (like, a lifetime) for this agent type'}
        # base_dict['_fcts'].update({'T_cycle': T_cycle_fcts})
        base_dict.update({'T_cycle_fcts': T_cycle_fcts})

        cycles_fcts = {
            'about': 'Number of times the sequence of periods/stages should be solved'}
        # base_dict['_fcts'].update({'cycle': cycles_fcts})
        base_dict.update({'cycles_fcts': cycles_fcts})
        cycles_fcts.update({'prmtv_par': 'True'})
        base_dict['prmtv_par'].append('cycles')
        
        self.parameters = base_dict
...
class init_idiosyncratic_shocks_plus_make(AgentDict):
    def __init__(self, init_idiosyncratic_shocks=init_idiosyncratic_shocks):
        base_params = init_perfect_foresight_plus_make(
            init_idiosyncratic_shocks).parameters

        base_params['prmtv_par'].append('permShkStd')
        base_params['prmtv_par'].append('tranShkStd')
        base_params['prmtv_par'].append('UnempPrb')
        base_params['prmtv_par'].append('UnempPrbRet')
        base_params['prmtv_par'].append('IncUnempRet')
        base_params['prmtv_par'].append('BoroCnstArt')
        base_params['prmtv_par'].append('tax_rate')
        base_params['prmtv_par'].append('T_retire')
        # Approximation parameters and their limits (if any)
        # base_params['aprox_par'].append('permShkCount')
        if not hasattr(init_idiosyncratic_shocks,'aprox_lim'):
            base_params['aprox_lim'] = {}
        base_params['aprox_lim'].update({'permShkCount': 'infinity'})
        # base_params['aprox_par'].append('tranShkCount')
        base_params['aprox_lim'].update({'tranShkCount': 'infinity'})
        # base_params['aprox_par'].append('aXtraMin')
        base_params['aprox_lim'].update({'aXtraMin': float('0.0')})
        # base_params['aprox_par'].append('aXtraMax')
        base_params['aprox_lim'].update({'aXtraMax': float('inf')})
        # base_params['aprox_par'].append('aXtraNestFac')
        base_params['aprox_lim'].update({'aXtraNestFac': None})
        # base_params['aprox_par'].append('aXtraCount')
        base_params['aprox_lim'].update({'aXtraCount': None})
        ...
        # base_params['_fcts'].update({'IncShkDstn': IncShkDstn_fcts})
        base_params.update({'IncShkDstn_fcts': IncShkDstn_fcts})

        permShkStd_fcts = {
            'about': 'Standard deviation for lognormal shock to permanent income'}
        permShkStd_fcts.update({'latexexpr': '\permShkStd'})
        # base_params['_fcts'].update({'permShkStd': permShkStd_fcts})
        base_params.update({'permShkStd_fcts': permShkStd_fcts})

        tranShkStd_fcts = {
            'about': 'Standard deviation for lognormal shock to permanent income'}
        tranShkStd_fcts.update({'latexexpr': '\tranShkStd'})
        # base_params['_fcts'].update({'tranShkStd': tranShkStd_fcts})
        base_params.update({'tranShkStd_fcts': tranShkStd_fcts})

        UnempPrb_fcts = {
            'about': 'Probability of unemployment while working'}
        UnempPrb_fcts.update({'latexexpr': r'\UnempPrb'})
        UnempPrb_fcts.update({'_unicode_': '℘'})
        # base_params['_fcts'].update({'UnempPrb': UnempPrb_fcts})
        base_params.update({'UnempPrb_fcts': UnempPrb_fcts})

        UnempPrbRet_fcts = {
            'about': '"unemployment" in retirement = big medical shock'}
        UnempPrbRet_fcts.update({'latexexpr': r'\UnempPrbRet'})
        # base_params['_fcts'].update({'UnempPrbRet': UnempPrbRet_fcts})
        base_params.update({'UnempPrbRet_fcts': UnempPrbRet_fcts})

        IncUnemp_fcts = {
            'about': 'Unemployment insurance replacement rate'}
        IncUnemp_fcts.update({'latexexpr': '\IncUnemp'})
        IncUnemp_fcts.update({'_unicode_': 'μ'})
        # base_params['_fcts'].update({'IncUnemp': IncUnemp_fcts})
        base_params.update({'IncUnemp_fcts': IncUnemp_fcts})

        IncUnempRet_fcts = {
            'about': 'Size of medical shock (frac of perm inc)'}
        # base_params['_fcts'].update({'IncUnempRet': IncUnempRet_fcts})
        base_params.update({'IncUnempRet_fcts': IncUnempRet_fcts})

        tax_rate_fcts = {
            'about': 'Flat income tax rate'}
        tax_rate_fcts.update({
            'about': 'Size of medical shock (frac of perm inc)'})
        # base_params['_fcts'].update({'tax_rate': tax_rate_fcts})
        base_params.update({'tax_rate_fcts': tax_rate_fcts})

        T_retire_fcts = {
            'about': 'Period of retirement (0 --> no retirement)'}
        # base_params['_fcts'].update({'T_retire': T_retire_fcts})
        base_params.update({'T_retire_fcts': T_retire_fcts})

        permShkCount_fcts = {
            'about': 'Num of pts in discrete approx to permanent income shock dstn'}
        # base_params['_fcts'].update({'permShkCount': permShkCount_fcts})
        base_params.update({'permShkCount_fcts': permShkCount_fcts})

        tranShkCount_fcts = {
            'about': 'Num of pts in discrete approx to transitory income shock dstn'}
        # base_params['_fcts'].update({'tranShkCount': tranShkCount_fcts})
        base_params.update({'tranShkCount_fcts': tranShkCount_fcts})

        aXtraMin_fcts = {
            'about': 'Minimum end-of-period "assets above minimum" value'}
        # base_params['_fcts'].update({'aXtraMin': aXtraMin_fcts})
        base_params.update({'aXtraMin_fcts': aXtraMin_fcts})

        aXtraMax_fcts = {
            'about': 'Maximum end-of-period "assets above minimum" value'}
        # base_params['_fcts'].update({'aXtraMax': aXtraMax_fcts})
        base_params.update({'aXtraMax_fcts': aXtraMax_fcts})

        aXtraNestFac_fcts = {
            'about': 'Exponential nesting factor when constructing "assets above minimum" grid'}
        # base_params['_fcts'].update({'aXtraNestFac': aXtraNestFac_fcts})
        base_params.update({'aXtraNestFac_fcts': aXtraNestFac_fcts})

        aXtraCount_fcts = {
            'about': 'Number of points in the grid of "assets above minimum"'}
        # base_params['_fcts'].update({'aXtraMax': aXtraCount_fcts})
        base_params.update({'aXtraMax_fcts': aXtraCount_fcts})

        aXtraCount_fcts = {
            'about': 'Number of points to include in grid of assets above minimum possible'}
        # base_params['_fcts'].update({'aXtraCount': aXtraCount_fcts})
        base_params.update({'aXtraCount_fcts': aXtraCount_fcts})

        aXtraExtra_fcts = {
            'about': 'List of other values of "assets above minimum" to add to the grid (e.g., 10000)'}
        # base_params['_fcts'].update({'aXtraExtra': aXtraExtra_fcts})
        base_params.update({'aXtraExtra_fcts': aXtraExtra_fcts})

        aXtraGrid_fcts = {
            'about': 'Grid of values to add to minimum possible value to obtain actual end-of-period asset grid'}
        # base_params['_fcts'].update({'aXtraGrid': aXtraGrid_fcts})
        base_params.update({'aXtraGrid_fcts': aXtraGrid_fcts})

        vFuncBool_fcts = {
            'about': 'Whether to calculate the value function during solution'
        }
        # base_params['_fcts'].update({'vFuncBool': vFuncBool_fcts})
        base_params.update({'vFuncBool_fcts': vFuncBool_fcts})

        CubicBool_fcts = {
            'about': 'Use cubic spline interpolation when True, linear interpolation when False'}
#        base_params['_fcts'].update({'CubicBool_fcts': CubicBool_fcts})
        base_params.update({'CubicBool_fcts': CubicBool_fcts})

        base_params.update({
            'matrx_par': {
                'mcrlo_aXtraCount': '100',
                'mcrlo_aXtraMin': base_params['aXtraMin'],
                'mcrlo_aXtraMax': base_params['aXtraMax']
            }})
        base_params.update({
            'matrx_lim': {
...
class init_kinked_R_plus_make(AgentDict):
    def __init__(self, base_dict):
        base_dict_updated = init_idiosyncratic_shocks_plus_make(
            base_dict).parameters
        del base_dict_updated["Rfree"]  # get rid of constant interest factor
        base_dict = base_dict_updated

class init_lifecycle_plus_make(AgentDict):
    def __init__(self, base_dict):
        base_dict_updated = init_idiosyncratic_shocks_plus_make(
            base_dict).parameters
        base_dict = base_dict_updated
