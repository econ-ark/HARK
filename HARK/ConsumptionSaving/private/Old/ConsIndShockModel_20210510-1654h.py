...
class Parameters(SimpleNamespace):
    """
    Parameters collected and preserved as arguments
    to various constructors, functions, and methods
...
#    distance_criteria = ["cFunc"]  # cFunc if the GIC fails
    ...
    def __init__(
        ...
        self.bilt = bilt if bilt is not None else Built()
...
class ConsumerSolutionOneStateCRRA(ConsumerSolution):
    ...
    def check_conditions(self, soln_crnt.bilt, verbose=None):
        """
        ...
        a nondegenerate soln_crnt.bilt. To check which conditions are required,
        ...
        soln_crnt.bilt : ConsumerSolution
        ...
        msg = msg+'\n'+str(soln_crnt.bilt.solve_par_vals)+'\n'

        if verbose >= 2:
            _log.info(msg)
            print(msg)  # Spyder does not make logs easy to watch, so print

        soln_crnt.bilt.check_AIC(soln_crnt.bilt, verbose)
        soln_crnt.bilt.check_FHWC(soln_crnt.bilt, verbose)
        soln_crnt.bilt.check_RIC(soln_crnt.bilt, verbose)
        soln_crnt.bilt.check_GICRaw(soln_crnt.bilt, verbose)
        soln_crnt.bilt.check_GICNrm(soln_crnt.bilt, verbose)
        soln_crnt.bilt.check_GICLiv(soln_crnt.bilt, verbose)
        soln_crnt.bilt.check_FVAC(soln_crnt.bilt, verbose)
        ...
        if hasattr(soln_crnt.bilt, "BoroCnstArt") \
            ...
            soln_crnt.bilt.degenerate = not soln_crnt.bilt.RIC
            # If BoroCnstArt exists but RIC fails, limiting soln is c(m)=0
        else:  # If no constraint,
            soln_crnt.bilt.degenerate = not soln_crnt.bilt.RIC or \
    ...
    def check_GICLiv(self, stge, verbose=None):
        ...
        messages = {
            True: "\nThe Mortality Adjusted Aggregate Growth Patience Factor for the supplied parameter values, GPFLiv={0.GPFLiv}, satisfies the Mortality Adjusted Aggregate Growth Imatience Condition (GICLiv):\n    "+stge.bilt.GPFLiv_fcts['urlhandle'],
            False: "\nThe Mortality Adjusted Aggregate Growth Patience Factor for the supplied parameter values, GPFLiv={0.GPFLiv}, violates the Mortality Adjusted Aggregate Growth Imatience Condition (GICLiv):\n    "+stge.bilt.GPFLiv_fcts['urlhandle'],
        }
        verbose_messages = {
    ...
    def check_FHWC(self, stge, verbose=None):
        ...
        verbose_messages = {
            True: "\n\n  Therefore, the limiting consumption function is not c(m)=Infinity ("+stge.bilt.FHWC_fcts['urlhandle']+")\n  Human wealth normalized by permanent income is {0.hNrmInf}.\n",
    ...
    def check_GICNrm(self, stge, verbose=None):
        ...
        verbose_messages = {
            True: "\n\n  Therefore, a target level of the individual market resources ratio m exists ("+stge.bilt.GICNrm_fcts['urlhandle']+").\n",
...
class ConsPerfForesightSolver(MetricObject):
    ...
    def __init__(
        ...
        if soln_futr.stge_kind['iter_status'] == 'terminal':
            self.soln_crnt.bilt = deepcopy(soln_futr)
        # Otherwise create receptacle for construction of solution
        else:
            self.soln_crnt.bilt = ConsumerSolutionOneStateCRRA()
        ...
        if hasattr(soln_futr.bilt, 'solve_par_vals'):
            self.soln_crnt.bilt.solve_par_vals =\
                soln_futr.bilt.solve_par_vals
    ...
    def def_utility_funcs(self, stge):
        ...
        # Inverses thereof
        stge.uPinv = lambda uP: utilityP_inv(uP, gam=stge.CRRA)
        stge.uPinvP = lambda uP: utilityP_invP(uP, gam=stge.CRRA)
        stge.uinvP = lambda uinvP: utility_invP(uinvP, gam=stge.CRRA)
        stge.uinv = lambda uinv: utility_inv(uinv, gam=stge.CRRA)  # in case vFuncBool
...
#        prms = stge.parameters_solver
        ...
        vFuncNvrsSlope = bilt.MPCmin ** (-bilt.CRRA / (1.0 - bilt.CRRA))
        vFuncNvrs = LinearInterp(
            np.array([bilt.mNrmMin, bilt.mNrmMin + 1.0]),
            np.array([0.0, vFuncNvrsSlope]),
        )
        bilt.vFunc = ValueFuncCRRA(vFuncNvrs, bilt.CRRA)
        bilt.vPfunc = MargValueFuncCRRA(stge.cFunc, bilt.CRRA)
        return stge
    ...
    def make_cFunc_PF(self):
        ...
        _tp1 = self.soln_crnt.bilt._tp1
        made = self.soln_crnt.bilt.scsr
...
#        mNrmGrid_tp1 = made.cFunc_tp1.x_list[:-1]
        mNrmGrid_tp1 = _tp1.cFunc_tp1.x_list[:-1]
#        cNrmGrid_tp1 = made.cFunc_tp1.y_list[:-1]
        cNrmGrid_tp1 = _tp1.cFunc_tp1.y_list[:-1]
        ...
        EvP_tp1 = (DiscLiv * Rfree) * \
            _tp1.uP_tp1(PermGroFac * cNrmGrid_tp1)  # Rβ E[u'(Γ c_tp1)]
        ...
        self.cFunc = self.soln_crnt.bilt.cFunc = bild.cFunc = LinearInterp(mNrmGrid, cNrmGrid)
    ...
    def build_infhor_facts_from_params_ConsPerfForesightSolver(self):
        ...
        soln_crnt.bilt = self.soln_crnt.bilt
        scsr = self.soln_crnt.bilt.scsr
        ...
        soln_crnt.bilt.APF = bilt.APF = APF = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'APF': APF_fcts})
        soln_crnt.bilt.APF_fcts = bilt.APF_fcts = APF_fcts
        ...
        # soln_crnt.bilt.fcts.update({'AIC': AIC_fcts})
        soln_crnt.bilt.AIC_fcts = bilt.AIC_fcts = AIC_fcts

        RPF_fcts = {
            'about': 'Return Patience Factor'
        }
        py___code = 'APF / Rfree'
        soln_crnt.bilt.RPF = bilt.RPF = RPF = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'RPF': RPF_fcts})
        soln_crnt.bilt.RPF_fcts = bilt.RPF_fcts = RPF_fcts
        ...
        # soln_crnt.bilt.fcts.update({'RIC': RIC_fcts})
        soln_crnt.bilt.RIC_fcts = bilt.RIC_fcts = RIC_fcts

        GPFRaw_fcts = {
            'about': 'Growth Patience Factor'
        }
        py___code = 'APF / PermGroFac'
        soln_crnt.bilt.GPFRaw = bilt.GPFRaw = GPFRaw = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'GPFRaw': GPFRaw_fcts})
        soln_crnt.bilt.GPFRaw_fcts = bilt.GPFRaw_fcts = \
            GPFRaw_fcts
        ...
        # soln_crnt.bilt.fcts.update({'GICRaw': GICRaw_fcts})
        soln_crnt.bilt.GICRaw_fcts = bilt.GICRaw_fcts = GICRaw_fcts

        GPFLiv_fcts = {
            'about': 'Mortality-Adjusted Growth Patience Factor'
        }
        py___code = 'APF * LivPrb / PermGroFac'
        soln_crnt.bilt.GPFLiv = bilt.GPFLiv = GPFLiv = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'GPFLiv': GPFLiv_fcts})
        soln_crnt.bilt.GPFLiv_fcts = bilt.GPFLiv_fcts = GPFLiv_fcts
        ...
        # soln_crnt.bilt.fcts.update({'GICLiv': GICLiv_fcts})
        soln_crnt.bilt.GICLiv_fcts = bilt.GICLiv_fcts = GICLiv_fcts

        PF_RNrm_fcts = {
            'about': 'Growth-Normalized PF Return Factor'
        }
        py___code = 'Rfree/PermGroFac'
        soln_crnt.bilt.PF_RNrm = bilt.PF_RNrm = PF_RNrm = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'PF_RNrm': PF_RNrm_fcts})
        soln_crnt.bilt.PF_RNrm_fcts = bilt.PF_RNrm_fcts = PF_RNrm_fcts
        soln_crnt.bilt.PF_RNrm = PF_RNrm

        Inv_PF_RNrm_fcts = {
            'about': 'Inv of Growth-Normalized PF Return Factor'
        }
        py___code = '1 / PF_RNrm'
        soln_crnt.bilt.Inv_PF_RNrm = bilt.Inv_PF_RNrm = Inv_PF_RNrm = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'Inv_PF_RNrm': Inv_PF_RNrm_fcts})
        soln_crnt.bilt.Inv_PF_RNrm_fcts = bilt.Inv_PF_RNrm_fcts = \
            Inv_PF_RNrm_fcts
        ...
        soln_crnt.bilt.FHWF = bilt.FHWF = FHWF = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'FHWF': FHWF_fcts})
        soln_crnt.bilt.FHWF_fcts = bilt.FHWF_fcts = \
            FHWF_fcts
        ...
        # soln_crnt.bilt.fcts.update({'FHWC': FHWC_fcts})
        soln_crnt.bilt.FHWC_fcts = bilt.FHWC_fcts = FHWC_fcts

        hNrmInf_fcts = {
            'about': 'Human wealth for inf hor'
        }
        py___code = '1/(1-FHWF) if (FHWF < 1) else float("inf")'
        soln_crnt.bilt.hNrmInf = bilt.hNrmInf = hNrmInf = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        hNrmInf_fcts = dict({'latexexpr': '1/(1-\FHWF)'})
        hNrmInf_fcts.update({'value_now': hNrmInf})
        hNrmInf_fcts.update({'py___code': py___code})
        # soln_crnt.bilt.fcts.update({'hNrmInf': hNrmInf_fcts})
        soln_crnt.bilt.hNrmInf_fcts = \
        ...
        soln_crnt.bilt.DiscGPFRawCusp = bilt.DiscGPFRawCusp = DiscGPFRawCusp = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'DiscGPFRawCusp': DiscGPFRawCusp_fcts})
        bilt.DiscGPFRawCusp_fcts = soln_crnt.bilt.DiscGPFRawCusp_fcts = \
            DiscGPFRawCusp_fcts
        ...
        soln_crnt.bilt.DiscGPFLivCusp = bilt.DiscGPFLivCusp = DiscGPFLivCusp = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'DiscGPFLivCusp': DiscGPFLivCusp_fcts})
        soln_crnt.bilt.DiscGPFLivCusp_fcts = bilt.DiscGPFLivCusp_fcts = DiscGPFLivCusp_fcts

        FVAF_fcts = {  # overwritten by version with uncertainty
            'about': 'Finite Value of Autarky Factor'
        }
        py___code = 'LivPrb * DiscLiv'
        soln_crnt.bilt.FVAF = bilt.FVAF = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        FVAF_fcts.update({'latexexpr': r'\FVAFPF'})
        FVAF_fcts.update({'urlhandle': urlroot+'FVAFPF'})
        FVAF_fcts.update({'py___code': py___code})
        # soln_crnt.bilt.fcts.update({'FVAF': FVAF_fcts})
        soln_crnt.bilt.FVAF_fcts = bilt.FVAF_fcts = FVAF_fcts
        ...
        # soln_crnt.bilt.fcts.update({'FVAC': FVAC_fcts})
        soln_crnt.bilt.FVAC_fcts = bilt.FVAC_fcts = FVAC_fcts

        Ex_IncNrmNxt_fcts = {  # Overwritten by version with uncertainty
            'about': 'Expected income next period'
        }
        py___code = '1.0'
        soln_crnt.bilt.Ex_IncNrmNxt = bilt.Ex_IncNrmNxt = Ex_IncNrmNxt = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
...
#        Ex_IncNrmNxt_fcts.update({'urlhandle': urlroot+'ExIncNrmNxt'})
        ...
        # soln_crnt.bilt.fcts.update({'Ex_IncNrmNxt': Ex_IncNrmNxt_fcts})
        soln_crnt.bilt.Ex_IncNrmNxt_fcts = soln_crnt.bilt.Ex_IncNrmNxt_fcts = Ex_IncNrmNxt_fcts
        ...
        soln_crnt.bilt.DiscLiv = bilt.DiscLiv = DiscLiv = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'DiscLiv': DiscLiv_fcts})
        soln_crnt.bilt.DiscLiv_fcts = bilt.DiscLiv_fcts = DiscLiv_fcts
    ...
    def build_recursive_facts_ConsPerfForesightSolver(self):
        ...
        if soln_crnt.bilt.stge_kind['iter_status'] == 'terminal':  # kludge:
            hNrm_tp1 = -1.0  # causes hNrm = 0 for final period
        soln_crnt.bilt.hNrm = bilt.hNrm = hNrm = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'hNrm': hNrm_fcts})
        soln_crnt.bilt.hNrm_fcts = bilt.hNrm_fcts = hNrm_fcts
        ...
        soln_crnt.bilt.BoroCnstNat = bilt.BoroCnstNat = BoroCnstNat = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'BoroCnstNat': BoroCnstNat_fcts})
        soln_crnt.bilt.BoroCnstNat_fcts = bilt.BoroCnstNat_fcts = BoroCnstNat_fcts
        ...
        soln_crnt.bilt.BoroCnst = bilt.BoroCnst = BoroCnst = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'BoroCnst': BoroCnst_fcts})
        soln_crnt.bilt.BoroCnst_fcts = bilt.BoroCnst_fcts = BoroCnst_fcts

        mNrmMin_fcts = {
            'about': 'Min m is the max you can borrow'
        }
        py___code = 'BoroCnst'
        soln_crnt.bilt.mNrmMin = bilt.mNrmMin =  \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        mNrmMin_fcts.update({'latexexpr': r'\mNrmMin'})
        mNrmMin_fcts.update({'py___code': py___code})
        # soln_crnt.bilt.fcts.update({'mNrmMin': mNrmMin_fcts})
        soln_crnt.bilt.mNrmMin_fcts = bilt.mNrmMin_fcts = mNrmMin_fcts
        ...
        soln_crnt.bilt.MPCmin = bilt.MPCmin = MPCmin = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'MPCmin': MPCmin_fcts})
        soln_crnt.bilt.MPCmin_fcts = bilt.MPCmin_fcts = MPCmin_fcts
        ...
        soln_crnt.bilt.MPCmax = bilt.MPCmax = MPCmax = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'MPCmax': MPCmax_fcts})
        ...
        soln_crnt.bilt.cFuncLimitIntercept = bilt.cFuncLimitIntercept = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
...
#        cFuncLimitIntercept_fcts.update({'cFuncLimitIntercept': cFuncLimitIntercept_fcts})
        ...
        bilt.cFuncLimitSlope = soln_crnt.bilt.cFuncLimitSlope = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
...
#        stg_crt.fcts.update({'cFuncLimitSlope': cFuncLimitSlope_fcts})
    ...
    def solve(self):  # ConsPerfForesightSolver
        ...
        if futr.stge_kind['iter_status'] == 'terminal':
            ...
            return soln_crnt.bilt  # if pseudo_terminal = True, enhanced replaces original
        ...
        self.make_cFunc_PF()
        self.soln_crnt.bilt = self.def_value_funcs(soln_crnt.bilt)

        return soln_crnt.bilt

    def solver_prepare_for_one_period_solution(self):  # self is solver for this stage of problem
        """
...
#        self.soln_crnt.bilt._tp1 = deepcopy(self.soln_futr.bilt)
        soln_crnt.bilt._tp1 = SuccessorInfo()

        # for avoid_recursion in {'solution__tp1', 'scsr'}:
        #     if hasattr(self.soln_crnt.bilt._tp1, avoid_recursion):
        #         delattr(self.soln_crnt.bilt._tp1, avoid_recursion)

        # Add '_tp1' to names of variables in self.soln_futr.bilt
        # and put in soln_crnt.bilt._tp1

        for key in (k for k in self.soln_futr.bilt.__dict__.keys()
                    if k not in {'solution_next', 'scsr', '_tp1'}):
            setattr(soln_crnt.bilt._tp1, key+'_tp1',
                    self.soln_futr.bilt.__dict__[key])
        ...
        # Now get the things that might or might not exist
        if hasattr(soln_futr.bilt, 'vFunc'):
            scsr.vFunc_tp1 = deepcopy(soln_futr.bilt.vFunc)

        if hasattr(soln_futr.bilt, 'vPPfunc'):
            scsr.vPPfunc_tp1 = deepcopy(soln_futr.bilt.vPPfunc)

        if hasattr(soln_futr.bilt, 'BoroCnstNat'):
    ...
    prepare_to_solve = solver_prepare_for_one_period_solution
...
class ConsIndShockSetup(ConsPerfForesightSolver):
    ...
    vFuncBool: boolean
        ...
        included in the reported soln_crnt.bilt.
    CubicBool: boolean
    ...
    def __init__(
        ...
        soln_crnt.bilt = self.soln_crnt.bilt  # ConsPerfForesightSolver.__init__ makes self.soln_crnt.bilt
        soln_crnt.bilt.IncShkDstn = IncShkDstn
    ...
    def build_infhor_facts_from_params(self):
        ...
        if soln_crnt.bilt.stge_kind['iter_status'] == 'terminal':  # kludge:
            MPCmax_tp1 = float('inf')  # causes MPCmax = 1 for final period
        soln_crnt.bilt.MPCmax = bilt.MPCmax = MPCmax = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        MPCmax_fcts.update({'latexexpr': r''})
        MPCmax_fcts.update({'urlhandle': urlroot+'MPCmax'})
        MPCmax_fcts.update({'py___code': py___code})
        MPCmax_fcts.update({'value_now': MPCmax})
        ...
        soln_crnt.bilt.Ex_IncNrmNxt = bilt.Ex_IncNrmNxt = Ex_IncNrmNxt = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        Ex_IncNrmNxt_fcts.update({'latexexpr': r'\Ex_IncNrmNxt'})
        ...
        soln_crnt.bilt.Ex_Inv_permShk = bilt.Ex_Inv_permShk = Ex_Inv_permShk = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
...
#        Ex_Inv_permShk_fcts.update({'_unicode_': r'R/Γ'})
        ...
        # soln_crnt.bilt.fcts.update({'Ex_Inv_permShk': Ex_Inv_permShk_fcts})
        soln_crnt.bilt.Ex_Inv_permShk_fcts = soln_crnt.bilt.Ex_Inv_permShk_fcts = Ex_Inv_permShk_fcts

        Inv_Ex_Inv_permShk_fcts = {
            'about': 'Inverse of Expected Inverse of Permanent Shock'
        }
        py___code = '1/Ex_Inv_permShk'
        soln_crnt.bilt.Inv_Ex_Inv_permShk = bilt.Inv_Ex_Inv_permShk = Inv_Ex_Inv_permShk = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'Inv_Ex_Inv_permShk': Inv_Ex_Inv_permShk_fcts})
        soln_crnt.bilt.Inv_Ex_Inv_permShk_fcts = soln_crnt.bilt.Inv_Ex_Inv_permShk_fcts = Inv_Ex_Inv_permShk_fcts
        # soln_crnt.bilt.Inv_Ex_Inv_permShk = Inv_Ex_Inv_permShk

        Ex_RNrm_fcts = {
            'about': 'Expected Stochastic-Growth-Normalized Return'
        }
        py___code = 'PF_RNrm * Ex_Inv_permShk'
        soln_crnt.bilt.Ex_RNrm = bilt.Ex_RNrm = Ex_RNrm = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'Ex_RNrm': Ex_RNrm_fcts})
        soln_crnt.bilt.Ex_RNrm_fcts = bilt.Ex_RNrm_fcts = Ex_RNrm_fcts

        Inv_Ex_RNrm_fcts = {
            'about': 'Inverse of Expected Stochastic-Growth-Normalized Return'
        }
        py___code = '1/Ex_RNrm'
        soln_crnt.bilt.Inv_Ex_RNrm = bilt.Inv_Ex_RNrm = Inv_Ex_RNrm = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'Inv_Ex_RNrm': Inv_Ex_RNrm_fcts})
        soln_crnt.bilt.Inv_Ex_RNrm_fcts = bilt.Inv_Ex_RNrm_fcts = Inv_Ex_RNrm_fcts

        Ex_uInv_permShk_fcts = {
            'about': 'Expected Utility for Consuming Permanent Shock'
        }
        py___code = '𝔼_dot(permShkValsBcst**(1-CRRA), ShkPrbs)'
        soln_crnt.bilt.Ex_uInv_permShk = bilt.Ex_uInv_permShk = Ex_uInv_permShk = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'Ex_uInv_permShk': Ex_uInv_permShk_fcts})
        soln_crnt.bilt.Ex_uInv_permShk_fcts = bilt.Ex_uInv_permShk_fcts = Ex_uInv_permShk_fcts

        uInv_Ex_uInv_permShk_fcts = {
            'about': 'Inverted Expected Utility for Consuming Permanent Shock'
        }
        py___code = '1/Ex_uInv_permShk'
        soln_crnt.bilt.uInv_Ex_uInv_permShk = bilt.uInv_Ex_uInv_permShk = uInv_Ex_uInv_permShk = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'uInv_Ex_uInv_permShk': uInv_Ex_uInv_permShk_fcts})
        soln_crnt.bilt.uInv_Ex_uInv_permShk_fcts = bilt.uInv_Ex_uInv_permShk_fcts = uInv_Ex_uInv_permShk_fcts

        PermGroFacAdj_fcts = {
            'about': 'Uncertainty-Adjusted Permanent Income Growth Factor'
        }
        py___code = 'PermGroFac * Inv_Ex_Inv_permShk'
        soln_crnt.bilt.PermGroFacAdj = bilt.PermGroFacAdj = PermGroFacAdj = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'PermGroFacAdj': PermGroFacAdj_fcts})
        soln_crnt.bilt.PermGroFacAdj_fcts = bilt.PermGroFacAdj_fcts = PermGroFacAdj_fcts

        GPFNrm_fcts = {
            'about': 'Normalized Expected Growth Patience Factor'
        }
        py___code = 'GPFRaw * Ex_Inv_permShk'
        soln_crnt.bilt.GPFNrm = bilt.GPFNrm = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'GPFNrm': GPFNrm_fcts})
        soln_crnt.bilt.GPFNrm_fcts = bilt.GPFNrm_fcts = GPFNrm_fcts
        ...
        # soln_crnt.bilt.fcts.update({'GICNrm': GICNrm_fcts})
        soln_crnt.bilt.GICNrm_fcts = bilt.GICNrm_fcts = GICNrm_fcts
        ...
        py___code = 'LivPrb * DiscLiv'
        soln_crnt.bilt.FVAF = bilt.FVAF = eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        soln_crnt.bilt.WRPF = bilt.WRPF = WRPF = eval(
            py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'WRPF': WRPF_fcts})
        soln_crnt.bilt.WRPF_fcts = bilt.WRPF_fcts = WRPF_fcts
        ...
        # soln_crnt.bilt.fcts.update({'WRIC': WRIC_fcts})
        soln_crnt.bilt.WRIC_fcts = bilt.WRIC_fcts = WRIC_fcts

        DiscGPFNrmCusp_fcts = {
            'about': 'DiscFac s.t. GPFNrm = 1'
        }
        py___code = '((PermGroFac*Inv_Ex_Inv_permShk)**(CRRA))/Rfree'
        soln_crnt.bilt.DiscGPFNrmCusp = bilt.DiscGPFNrmCusp = DiscGPFNrmCusp = \
            eval(py___code, {}, {**bilt.__dict__, **_tp1.__dict__})
        ...
        # soln_crnt.bilt.fcts.update({'DiscGPFNrmCusp': DiscGPFNrmCusp_fcts})
        soln_crnt.bilt.DiscGPFNrmCusp_fcts = bilt.DiscGPFNrmCusp_fcts = DiscGPFNrmCusp_fcts
    ...
    def build_recursive_facts(self):
        ...
        Ex_m_tp1_minus_m_t = (
            lambda m_t:
            soln_crnt.bilt.Ex_RNrm * (m_t - soln_crnt.bilt.cFunc(m_t)) +
            soln_crnt.bilt.Ex_IncNrmNxt - m_t
        )
        soln_crnt.bilt.Ex_m_tp1_minus_m_t = Ex_m_tp1_minus_m_t

        soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_num_a_t = (
            lambda a_t:
            𝔼_dot(
                soln_crnt.bilt.permShkValsBcst * soln_crnt.bilt.PermGroFac * soln_crnt.bilt.cFunc(
        ...
        soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_lst_a_t = (
            lambda a_lst: list(map(
                soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_aNrm_num, a_lst
            ))
        )

        soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_a_t = (
            lambda a_t:
            soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_lst_a_t(a_t)
            if (type(a_t) == list or type(a_t) == np.ndarray) else
            soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_num_a_t(a_t)
        )

        soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_lst_m_t = (
            lambda m_t:
            soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_lst_a_t(m_t -
                                                                soln_crnt.bilt.cFunc(m_t))
        )

        soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_num_m_t = (
            lambda m_t:
            soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_num_a_t(m_t -
                                                                soln_crnt.bilt.cFunc(m_t))
        )
        ...
        soln_crnt.bilt.Ex_cLev_tp1_Over_cLev_t_from_m_t = (
            lambda m_t:
            soln_crnt.bilt.Ex_cLev_tp1_Over_pLev_t_from_m_t(m_t) /
            soln_crnt.bilt.cFunc(m_t)
        )
        soln_crnt.bilt.Ex_permShk_tp1_times_m_tp1_minus_m_t = (
            lambda m_t:
            soln_crnt.bilt.PF_RNrm *
            (m_t - soln_crnt.bilt.cFunc(m_t)) + 1.0 - m_t
        )

        self.soln_crnt.bilt = soln_crnt.bilt
...
class ConsIndShockSolverBasic(ConsIndShockSetup):
    ...
    def calc_EndOfPrdvP(self):
        ...
        def vP_tp1(shk_vector, a_number):
            return shk_vector[0] ** (-bilt.CRRA) \
                * self.soln_crnt.bilt._tp1.vPfunc_tp1(self.m_Nrm_tp1(shk_vector, a_number))
    ...
    def make_sol_using_EGM(self):  # Endogenous Gridpts Method
        ...
        Parameters
        ----------
        none (relies upon self.soln_crnt.bilt.aNrm existing before invocation)
        ...
        if self.soln_crnt.bilt.CubicBool:
            soln_crnt.bilt = self.interpolating_EGM_solution(
    ...
    def solve(self):  # solves ONE stage
        ...
        if futr.stge_kind['iter_status'] == 'terminal':
            ...
            soln_crnt.bilt.stge_kind['iter_status'] = 'iterator'
            return soln_crnt.bilt  # Replaces original "terminal" solution; next soln_futr
        ...
        soln_crnt.bilt.cFunc = soln_crnt.bilt.cFunc = sol_EGM.cFunc
        soln_crnt.bilt.vPfunc = soln_crnt.bilt.vPfunc = sol_EGM.vPfunc
        ...
        if soln_crnt.bilt.vFuncBool:
            soln_crnt.bilt.vFunc = self.vFunc = self.add_vFunc(soln_crnt.bilt, self.EndOfPrdvP)
        if soln_crnt.bilt.CubicBool:
            soln_crnt.bilt.vPPfunc = self.add_vPPfunc(soln_crnt.bilt)
...
class ConsIndShockSolver(ConsIndShockSolverBasic):
    ...
    def make_cubic_cFunc(self, mNrm_Vec, cNrm_Vec):
        ...
        def vPP_tp1(shk_vector, a_number):
            return shk_vector[0] ** (- bilt.CRRA - 1.0) \
                * scsr.vPPfunc_tp1(self.m_Nrm_tp1(shk_vector, a_number))
    ...
    def make_EndOfPrdvFunc(self, EndOfPrdvP):
        ...
        EndOfPrdvP : np.array
            ...
            asset values in self.soln_crnt.bilt.aNrm.
        ...
        def v_Lvl_tp1(shk_vector, a_number):
            ...
            ) * bilt.vFuncNxt(self.soln_crnt.bilt.m_Nrm_tp1(shk_vector, a_number))
        EndOfPrdv = bilt.DiscLiv * calc_expectation(
            bilt.IncShkDstn, v_Lvl_tp1, self.soln_crnt.bilt.aNrm
        )
        EndOfPrdvNvrs = self.soln_crnt.bilt.uinv(
            EndOfPrdv
        )  # value transformed through inverse utility
        EndOfPrdvNvrsP = EndOfPrdvP * self.soln_crnt.bilt.uinvP(EndOfPrdv)
        EndOfPrdvNvrs = np.insert(EndOfPrdvNvrs, 0, 0.0)
        EndOfPrdvNvrsP = np.insert(
            EndOfPrdvNvrsP, 0, EndOfPrdvNvrsP[0]
        )  # This is a very good approximation, vNvrsPP = 0 at the asset minimum
        aNrm_temp = np.insert(self.soln_crnt.bilt.aNrm, 0, self.soln_crnt.bilt.BoroCnstNat)
        EndOfPrdvNvrsFunc = CubicInterp(aNrm_temp, EndOfPrdvNvrs, EndOfPrdvNvrsP)
        self.soln_crnt.bilt.EndOfPrdvFunc = ValueFuncCRRA(
            EndOfPrdvNvrsFunc, bilt.CRRA)

    def add_vFunc(self, soln_crnt.bilt, EndOfPrdvP):
        """
        Creates the value function for this period and adds it to the soln_crnt.bilt.
        ...
        self.vFunc = soln_crnt.bilt.vFunc = self.make_vFunc(soln_crnt.bilt)
        return soln_crnt.bilt.vFunc

    def make_vFunc(self, soln_crnt.bilt):
        """
        ...
        cNrm = soln_crnt.bilt.cFunc(mNrm_temp)
        aNrm = mNrm_temp - cNrm
    ...
    def add_vPPfunc(self, soln_crnt.bilt):
        """
...
class ConsKinkedRsolver(ConsIndShockSolver):
    ...
    it terminates immediately if Rboro < Rsave, as this has a different soln_crnt.bilt.
    ...
    def agent_update_if_params_have_changed_since_last_solve(self):
        ...
        if hasattr(self, 'solve_par_vals'):
            ...
            if not solve_par_vals_now == self.solve_par_vals:
                ...
                self.agent_force_prepare_info_needed_for_solver()  # The AgentType must define its own

    # update = agent_update_if_params_have_changed_since_last_solve
#                update = agent_update_if_params_have_changed_since_last_solve

    def agent_force_prepare_info_needed_for_solver(self):
    ...
    pre_solve = agent_force_prepare_info_needed_for_solver
...
class OneStateConsumerType(AgentTypePlus):
    ...
    def __init__(
        ...
        self.soln_crnt.bilt = ConsumerSolutionOneStateCRRA()  # Mainly for storing functions, methods
...
class PerfForesightConsumerType(OneStateConsumerType):
    ...
    def __init__(self,
        ...
        # OneStateConsumerType creates self.soln_crnt.bilt and self.soln_crnt.bilt.scsr
        ...
        if not hasattr(self, 'solution_startfrom'):
            ...
            soln_crnt.bilt = self.finish_setup_default_term_by_putting_into_soln_crnt.bilt()
            ...
            soln_crnt.bilt = solution_startfrom
        ...
        # soln_crnt.bilt.parameters_solver = \
    ...
    def make_solution_for_final_period(self):  # solution[0]=terminal_solution
        ...
        self.cycles_orig = deepcopy(self.cycles)  # and true cycles
    ...
    def check_conditions(self, verbose):
        ...
        soln_crnt.bilt = self.solution[0]
        soln_crnt.bilt.check_conditions(soln_crnt.bilt, verbose)
    ...
    def finish_setup_default_term_by_putting_into_soln_crnt.bilt(self):
        """
        ...
        # which is nobequest
        soln_crnt.bilt = self.soln_crnt.bilt = self.solution_terminal

        # Put these at root of solution
        soln_crnt.bilt.time_vary = self.time_vary = deepcopy(self.time_vary_)
        soln_crnt.bilt.time_inv = self.time_inv = deepcopy(self.time_inv_)
        soln_crnt.bilt.cycles = self.cycles

        # Natural borrowing constraint: Cannot die in debt
        # Measured after tranShk received
        soln_crnt.bilt.BoroCnstNat = soln_crnt.bilt.BoroCnstNat = \
            -soln_crnt.bilt.hNrm
        ...
        self.vFunc = soln_crnt.bilt.vFunc = soln_crnt.bilt.vFunc = ValueFuncCRRA(
            soln_crnt.bilt.cFunc, self.CRRA)
        self.vPfunc = soln_crnt.bilt.vPfunc = soln_crnt.bilt.vPfunc = MargValueFuncCRRA(
            soln_crnt.bilt.cFunc, self.CRRA)
        self.vPPfunc = soln_crnt.bilt.vPPfunc = soln_crnt.bilt.vPPfunc = MargMargValueFuncCRRA(
            soln_crnt.bilt.cFunc, self.CRRA)

        # utility u, marginal utility u' is uP, marginal marginal uPP
        soln_crnt.bilt.u = soln_crnt.bilt.u = lambda c: utility(c, self.CRRA)
        soln_crnt.bilt.uP = soln_crnt.bilt.uP = lambda c: utilityP(c, self.CRRA)
        soln_crnt.bilt.uPP = soln_crnt.bilt.uPP = lambda c: utilityPP(c, self.CRRA)

        # Inverses and pseudo-inverses
        soln_crnt.bilt.uPinv = soln_crnt.bilt.uPinv = lambda u: utilityP_inv(u, self.CRRA)
        soln_crnt.bilt.uPinvP = soln_crnt.bilt.uPinvP = lambda u: utilityP_invP(u, self.CRRA)
        soln_crnt.bilt.uinvP = soln_crnt.bilt.uinvP = lambda u: utility_invP(u, self.CRRA)
        soln_crnt.bilt.uinv = soln_crnt.bilt.uinv = lambda u: utility_inv(u, self.CRRA)

        soln_crnt.bilt.stge_kind = {'iter_status': 'terminal',
                                    'slvr_type': 'ConsPerfForesightSolver'
                                    }

        # Natural borrowing constraint: Cannot die in debt
        soln_crnt.bilt.mNrmMin = deepcopy(soln_crnt.bilt.mNrmMin)
        soln_crnt.bilt.hNrm = deepcopy(soln_crnt.bilt.hNrm)
        soln_crnt.bilt.MPCmin = deepcopy(soln_crnt.bilt.MPCmin)
        soln_crnt.bilt.MPCmax = deepcopy(soln_crnt.bilt.MPCmax)
        soln_crnt.bilt.BoroCnstNat = deepcopy(soln_crnt.bilt.BoroCnstNat)
    ...
    def agent_force_prepare_info_needed_for_solver(self):
        ...
        # overridden by the agent_force_prepare_info_needed_for_solver
    ...
    pre_solve = agent_force_prepare_info_needed_for_solver  # Alias to preserve functionality
...
class IndShockConsumerType(PerfForesightConsumerType):
    ...
    def __init__(self, cycles=1, verbose=1,  quiet=True, solution_startfrom=None, **kwds):
        ...
        PerfForesightConsumerType.__init__(
            ...
            #            solver=ConsPerfForesightSolver,  # Default if no solver supplied
            **params
        )

        # Reduce clutter with local variable (no annoying 'self.' required)
        soln_crnt.bilt = self.soln_crnt.bilt  # Already created by PerfForesightConsumerType
        ...
        if not hasattr(self, 'solution_startfrom'):  # .. then init the default
            ...
            self.agent_force_prepare_info_needed_for_solver()
        ...
        soln_crnt.bilt.stge_kind['slvr_type'] = 'ConsIndShockSolver'

        # Attach the corresponding one-stage solver to the agent
        self.solve_one_period = make_one_period_oo_solver(solver)

        # Add solver args so solution soln_crnt.bilt knows how it was made
    ...
    def agent_force_prepare_info_needed_for_solver(self):  # Alias below
        """
    ...
    # The former "[AgentType].update_pre_solve()" was poor nomenclature -- easy to confuse with
    # the also-existing "[AgentType].pre_solve()" and with
    # "[SolverType].prepare_to_solve()".  The new name,
    #
    # agent_force_prepare_info_needed_for_solver()
    #
    # is better, but the old one
    # is preserved as an alias to prevent breakage of existing code:
    pre_solve = agent_force_prepare_info_needed_for_solver
    ...
    def update_income_process(self):
        ...
        soln_crnt.bilt.IncShkDstn = self.IncShkDstn = IncShkDstn
        soln_crnt.bilt.permShkDstn = self.permShkDstn = permShkDstn
        soln_crnt.bilt.tranShkDstn = self.tranShkDstn = tranShkDstn
...
class KinkedRconsumerType(IndShockConsumerType):
    ...
    def __init__(self, cycles=1, **kwds):
        ...
        self.agent_update_if_params_have_changed_since_last_solve()
