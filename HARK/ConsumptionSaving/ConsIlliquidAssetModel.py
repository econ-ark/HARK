...
from HARK.interpolation import ( 
    LinearInterp, LinearInterpOnInterp1D, MargValueFuncCRRA,
    VariableLowerBoundFunc2D, IdentityFunction, LowerEnvelope2D
    )
...
class DepositFunction(MetricObject):
    ...
    None
    """
    distance_criteria = ['LowerBound', 'UpperBound', 'CashDeposit', 'CashWithdraw', 'Penalty']
    ...
    def __call__(self,mNrm,nNrm):
        ...
        oNrmAdj = mNrm[withdraw] + nNrm[withdraw] / (1. + self.Penalty)
...
def calc_marg_values_next(Y, a, b, rho, Rboro, Rsave, Rilqd, Gamma, marg_value_func_next):
    """
...
def solve_one_period_basic_illiquid(
    ...
    BoroCnstNat = calc_boro_const_nat(solution_next.mNrmMin,
                                      bNrmGrid,
                                      IncShkDstn,
                                      Rboro,
                                      PermGroFac)
    ...
    EndOfPrd_dvda, EndOfPrd_dvdb = calc_marg_values_next(
                          IncShkDstn, aNrmNow, bNrmNow, CRRA, Rboro, Rsave,
                          Rilqd, PermGroFac, MargValueFuncNext)
    ...
    RatioTarg = np.log(1. + IlqdPenalty)
    for j in range(bCount):
        ...
        OptZeroDeposit[j] = brentq(LogRatioFunc, kBot, kTop, xtol=1e-8, rtol=1e-8) + BoroCnstNat[j]
        OptZeroWithdraw[j] = brentq(LogRatioFuncAlt, kBot, kTop, xtol=1e-8, rtol=1e-8) + BoroCnstNat[j]
    ...
    MargValueFuncNow = SpecialMargValueFunction(dFuncNow, dvdkFunc, dvdbFunc, IlqdPenalty)
