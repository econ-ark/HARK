...
class LinearFast(MetricObject):
    ...
    def __call__(self, *args):
        ...
        f = eval_linear(
            self.Grid, self.f_val, np.column_stack([x.flatten() for x in array_args])
        )
...
class RiskyAssetConsumerType(IndShockConsumerType):
    ...
    def simBirth(self, which_agents):
        ...
        self.state_now["ShareNow"][which_agents] = 0.0
...
class RiskyContribConsumerType(RiskyAssetConsumerType):
    ...
    def simBirth(self, which_agents):
        ...
        IndShockConsumerType.simBirth(self, which_agents)
        self.state_now["nNrmTildeNow"][which_agents] = 0.0
        self.state_now["ShareNow"][which_agents] = 0.0
    ...
    def getStatesReb(self):
        ...
        # Permanent income
        self.state_now["pLvlNow"] = pLvlPrev * self.shocks["PermShkNow"]

        # Assets: mNrm and nNrm

        # Compute the effective growth factor of each asset
        RfEffNow = RfreeNow / self.shocks["PermShkNow"]
        RrEffNow = RriskNow / self.shocks["PermShkNow"]
        ...
        self.state_now["mNrmNow"] = bNrmNow + self.shocks["TranShkNow"] * (
            1 - self.state_now["ShareNow"]
        )
    ...
    def getStatesSha(self):
        ...
        if not "tau" in self.time_vary:
            ...
            mNrmTildeNow, nNrmTildeNow = rebalanceAssets(
                self.controls["DNrmNow"],
            ...
            for t in range(self.T_cycle):
                ...
                if np.sum(these) > 0:
                    ...
                    mNrmTildeNow[these], nNrmTildeNow[these] = rebalanceAssets(
                        self.controls["DNrmNow"][these],
    ...
    def getPostStates(self):
        ...
        self.state_now["aNrmNow"] = (
            self.state_now["mNrmTildeNow"] - self.controls["cNrmNow"]
        )
    ...
    for nInd in range(nNrm_N):
        for sInd in range(Share_N):
            ...
            if m_ns[0] == 0.0:
                ...
                # c
                c_vals[:, nInd, sInd] = LinearFast(c_end[:, nInd, sInd], m_ns)(mNrmGrid)
...
def solveRiskyContribShaStage(
    ...
    **unused_params
    ...
    if DiscreteShareBool:
        ShareFunc = DiscreteInterp2D(LinearFast(optIdx, mNrmGrid, nNrmGrid), ShareGrid)
    else:
...
def solveRiskyContribRebStage(
    ...
    # Marginals
    dvdmFuncAdj = MargValueFuncCRRA(LinearFast(dvdmNvrsAdj, mNrmGrid, nNrmGrid), CRRA)
    dvdnFuncAdj = MargValueFuncCRRA(LinearFast(dvdnNvrsAdj, mNrmGrid, nNrmGrid), CRRA)
