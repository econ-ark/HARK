...
#            self.soln_stge_add_further_info_ConsPerfForesightSolver(soln_stge)
     ...
     # soln_stge.PermShkValsNxtXref = PermShkValsNxtXref = self.soln_stge.IncShkDstn.X[0]
     # soln_stge.TranShkValsNxtXref = TranShkValsNxtXref = self.soln_stge.IncShkDstn.X[1]
     # soln_stge.ShkPrbsNext = ShkPrbsNext = self.soln_stge.IncShkDstn.pmf

     # soln_stge.PermShkValsNxt = PermShkValsNxt = self.soln_stge.PermShkDstn.X
     # soln_stge.TranShkValsNxt = TranShkValsNxt = self.soln_stge.TranShkDstn.X
     # PermShkMin = np.min(PermShkValsNxt)
     # TranShkMin = np.min(TranShkValsNxt)
...
#        Inv_Ex_RNrm_facts.update({'_unicode_': r'R/Γ'})
        ...
        soln_stge.Ex_cLev_tp1_Over_pLev_t_from_at = Ex_cLev_tp1_Over_pLev_t_from_at = (
            lambda a_t:
            np.dot(
                soln_stge.PermShkValsNxtXref * soln_stge.PermGroFac_0_ * soln_stge.cFunc(
