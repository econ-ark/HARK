...
```yaml
source_version: "0.14.1"      # e.g., "0.14.1"
target_version: "0.17.0"      # e.g., "0.16.1"
...
Produce a **Change Inventory**: a structured, evidence-backed document listing every breaking change between `0.14.1` and `0.17.0`.
...
git diff 0.14.1..0.17.0 -- HARK/distributions/*.py | grep -E "^\+.*self\.|^\-.*self\."
```
...
**Old signature** (≤0.14.1):
```python
def __init__(self, pmf, X, seed=0)
```

**New signature** (0.17.0):
```python
...
User code may use patterns from versions OLDER than `0.14.1` due to backward-compatibility shims.
...
git diff 0.14.1..0.17.0 -- examples/
git diff 0.14.1..0.17.0 -- HARK/tests/

# If DemARK repo is available
git diff 0.14.1..0.17.0 -- notebooks/
```
...
|------|------------|-----------------|----------------|
| `binder/requirements.txt` | `econ-ark==X.Y.Z` | `0.14.1` | `0.17.0` |
| `pyproject.toml` | `dependencies` | ... | ... |

**Migration Rule**:
- Pattern: `econ-ark[=<>~!]*{SOURCE_VERSION_PATTERN}`
- Replace with: `econ-ark==0.17.0` (or appropriate specifier for ranges)
...
```markdown
# Migration Checklist: HARK 0.14.1 → 0.17.0
...
- `binder/requirements.txt`: ✅ Updated to `econ-ark==0.17.0`
