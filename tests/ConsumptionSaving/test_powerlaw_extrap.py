...
CE_PARS = dict(
    cycles=0, T_cycle=1, CRRA=2.0, Rfree=[1.04], DiscFac=0.96,
    LivPrb=[1.0], PermGroFac=[1.0], BoroCnstArt=None,
    vFuncBool=False, CubicBool=False,
    UnempPrb=0.05, IncUnemp=0.0, TranShkStd=[0.1], TranShkCount=7,
    PermShkStd=[0.0], PermShkCount=1,
...
HS_PARS = dict(
    cycles=0, T_cycle=1, CRRA=2.0, Rfree=[1.01], DiscFac=0.98051,
    LivPrb=[1.0 - 1.0 / 160.0], PermGroFac=[1.0 + 0.01812 / 4],
    BoroCnstArt=None, vFuncBool=False, CubicBool=False,
    UnempPrb=0.044, IncUnemp=0.7, TranShkStd=[np.sqrt(0.12)], TranShkCount=7,
    PermShkStd=[np.sqrt(0.003)], PermShkCount=7,
    T_retire=0, UnempPrbRet=0.0, IncUnempRet=0.0,
)
...
BYTE_ZERO_PIN_A = [
    0.5, 0.8652278427790324, 1.098045258173503, 1.3730629141572253,
    1.6897273979915504, 2.234807386872702, 3.2691485935849434,
]
# config B: cycles=0, CubicBool=True, vFuncBool=True, BoroCnstArt=None
BYTE_ZERO_PIN_B = [
    1.0194785800743766, 1.08444079226619, 1.1836154886611796,
    1.405144565610918, 1.7072978107134946, 2.2460708318213296,
    3.269100840528629,
]
...
def attach_eval_only(solution, q_eff, CRRA, do_top=True, do_bottom=True):
    ...
    if do_top:
        f = DecayTailInterp(
            f, solution.MPCmin * solution.hNrm, solution.MPCmin,
            x_cut=float(m[-1]), decay_extrap_Q=q_eff,
        )
    if do_bottom:
        ...
        f = KappaBarTailInterp(
            f, solution.MPCmax, CRRA, solution.mNrmMin,
...
def ce_bundle():
    ...
    tails = dict(decay_extrap_form="powerlaw",
    ...
    grid_aud = log_grid(CE_GRID["aMin"] * 1e-4, CE_GRID["aMax"],
                        CE_GRID["Na"] + int(4 * npd))
...
def hs_bundle():
    ...
    grid_aud = log_grid(HS_GRID["aMin"], HS_GRID["aMax"] * 1e2,
                        HS_GRID["Na"] + int(2 * npd))
...
def top_window(truth_sol, small_sol, n=80):
    ...
    return np.exp(np.linspace(np.log(m_top_s * 1.05),
                              np.log(m_top_t * 0.95), n))


def interior_window(truth_sol, small_sol, n=200):
    mm = max(truth_sol.mNrmMin, small_sol.mNrmMin)
    m1 = max(float(unwrap_m_nodes(truth_sol.cFunc)[1]),
             float(unwrap_m_nodes(small_sol.cFunc)[1]))
    m_top_s = float(unwrap_m_nodes(small_sol.cFunc)[-1])
    return mm + np.exp(np.linspace(np.log((m1 - mm) * 2.0),
                                   np.log((m_top_s - mm) / 2.0), n))
...
class TestKappaBarTailInterpUnit(unittest.TestCase):
    ...
    def test_hand_computed_tail_values_and_delegation(self):
        ...
        np.testing.assert_allclose(
            np.asarray(f(np.array([0.5, 0.75])), float), [0.35, 0.525],
            rtol=1e-15)
        ...
        f2 = KappaBarTailInterp(self.body(), 0.8, 2.0, 0.0, x_knot=0.5,
                                y_knot=0.35)
    ...
    def test_guards_strict_corridor_and_exposure_gate(self):
        ...
        self.assertIsNone(
            KappaBarTailInterp.try_make(body, 0.8, 2.0, 0.0, x_knot=1.0))
        ...
        with self.assertRaises(ValueError):
            KappaBarTailInterp(body_far, 0.8, 2.0, 0.0, x_knot=1.0,
                               strict=False)
        self.assertIsNone(
        ...
        ok = KappaBarTailInterp.try_make(self.body(), 0.8, 2.0, 0.0,
                                         x_knot=0.5)
        self.assertIsNotNone(ok)
        self.assertTrue(ok.in_regime)
        self.assertGreater(ok.mpc_at_knot, 0.0)
        # malformed scalars fail closed (None), never raise
        self.assertIsNone(KappaBarTailInterp.try_make(
            self.body(), np.nan, 2.0, 0.0, x_knot=0.5))
        self.assertIsNone(KappaBarTailInterp.try_make(
            self.body(), 0.8, 2.0, 0.6, x_knot=0.5))  # knot below mNrmMin
...
class TestCePsiRegime(unittest.TestCase):
    def test_regime_I_on_ce_process(self):
        ...
        self.assertAlmostEqual(reg["lambda_min_fiber"], 0.05**0.5 * PG,
                               places=12)
    ...
    def test_regime_II_on_fat_low_fiber(self):
        ...
        self.assertAlmostEqual(reg["lambda_min_fiber"],
                               0.3**0.5 * PG / 0.12, places=10)
...
class TestAXtraMinFromTailTol(unittest.TestCase):
    ...
    def test_fail_closed(self):
        ...
        self.assertTrue(np.isnan(aXtraMin_from_tail_tol(0.5, 0.45, 0.8, 2.0,
                                                        1e-4)))
        self.assertTrue(np.isnan(aXtraMin_from_tail_tol(0.5, 0.35, 1.2, 2.0,
                                                        1e-4)))
        self.assertTrue(np.isnan(aXtraMin_from_tail_tol(0.5, 0.35, 0.8, 2.0,
                                                        np.nan)))
        # tail_tol clamped at the float64-certifiable floor 1e-6
        self.assertEqual(aXtraMin_from_tail_tol(0.5, 0.35, 0.8, 2.0, 1e-9),
    ...
    def test_expost_certificate_on_ce_solve(self):
        ...
        self.assertLessEqual(measured, tail_tol,
...
class TestOptionSurface(unittest.TestCase):
    ...
    def test_auto_exponent_is_min_one_qstar(self):
        ...
        self.assertEqual(agent_hs.decay_extrap_Q,
    ...
    def test_solver_requires_explicit_Q(self):
        ...
        with self.assertRaises(ValueError):
            solve_one_period_ConsIndShock_with_tails(
                agent.solution[0], agent.IncShkDstn[0], 1.0, 0.96, 2.0, 1.04,
                1.0, None, agent.aXtraGrid, False, False,
    ...
    def test_public_solver_signature_unchanged(self):
        ...
        legacy = list(
            inspect.signature(solve_one_period_ConsIndShock).parameters)
        self.assertEqual(legacy, [
            "solution_next", "IncShkDstn", "LivPrb", "DiscFac", "CRRA",
            "Rfree", "PermGroFac", "BoroCnstArt", "aXtraGrid", "vFuncBool",
            "CubicBool",
        ])
        # ... and the options swap the solver in only when enabled
        ce = ce_bundle()
        self.assertIs(ce[("tails", CE_K)].solve_one_period,
                      solve_one_period_ConsIndShock_with_tails)
        self.assertIs(ce[("rails", CE_K)].solve_one_period,
                      solve_one_period_ConsIndShock)
...
class TestByteZeroDefault(unittest.TestCase):
    ...
    def test_cubic_vfunc_natural_constraint_unchanged(self):
        agent = IndShockConsumerType(cycles=0, CubicBool=True, vFuncBool=True,
                                     BoroCnstArt=None)
        agent.verbose = 0
        agent.solve()
        got = np.asarray(agent.solution[0].cFunc(BYTE_ZERO_PROBES), float)
    ...
    def test_explicit_none_options_identical_to_stock(self):
        ...
        a.verbose = 0
        a.solve()
        b = IndShockConsumerType(cycles=0, decay_extrap_form=None,
                                 decay_extrap_Q=None,
...
class TestCEFidelity(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.floor_bottom = sup_rel_err(cls.ce["audit"].solution[0].cFunc,
                                       cls.truth.cFunc, cls.pb)

    def test_subset_identity(self):
        grid = self.ce["grid"]
        np.testing.assert_array_equal(
            np.asarray(self.ce[("tails", CE_K)].aXtraGrid),
            grid[CE_K:-CE_K])
    ...
    def test_audit_non_circularity(self):
        ...
        self.assertLess(self.floor_bottom, GATE_CE_AUDIT,
                        "audit floor %.3e vs gate %.1e" %
                        (self.floor_bottom, GATE_CE_AUDIT))

    def test_bottom_fidelity_gate(self):
        # floor discipline: the gate must sit above the printed audit floor
        self.assertGreater(GATE_CE_BOTTOM, 10.0 * self.floor_bottom,
    ...
    def test_bottom_beats_rails(self):
        ...
        self.assertGreater(err_r, 10.0 * err_x,
                           "rails %.3e vs tails %.3e" % (err_r, err_x))
    ...
    def test_bottom_mpc_approaches_kappa_bar(self):
        ...
        print("CE MPC at m=%.3e: %.12f vs MPCmax %.12f (dev %.2e)"
    ...
    def test_top_fidelity_gate(self):
        ...
        print("CE top: tails %.3e rails %.3e (gate %.1e)"
              % (err, err_r, GATE_CE_TOP))
    ...
    def test_two_roles_bottom(self):
        ...
        print("CE two-roles bottom: in-solve %.3e vs eval-only %.3e "
              "(factor %.1fx, floor %.3e)"
    ...
    def test_in_grid_contamination(self):
        ...
        print("CE contamination: tails %.3e rails %.3e (gate %.1e)"
    ...
    def test_ladder_monotone(self):
        ...
        print("CE ladder bottom: k=%d %.3e <= k=%d %.3e"
    ...
    def test_determinism(self):
        ...
        again = solve_agent(CE_PARS, grid, SOLVE_TOL,
...
class TestHSFidelity(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.pt = top_window(cls.truth, cls.tails)
        cls.pb = bottom_window(cls.truth, cls.tails)
        cls.floor_top = sup_rel_err(cls.hs["audit"].solution[0].cFunc,
                                    cls.truth.cFunc, cls.pt)

    def test_audit_non_circularity(self):
        # G-AUD3 port: rails-only on the 2-decades-taller grid vs truth
        self.assertLess(self.floor_top, GATE_HS_AUDIT,
                        "audit floor %.3e vs gate %.1e" %
                        (self.floor_top, GATE_HS_AUDIT))

    def test_top_fidelity_gate(self):
        self.assertGreater(GATE_HS_TOP, 10.0 * self.floor_top,
                           "gate %.1e not >= 10x audit floor %.3e" %
                           (GATE_HS_TOP, self.floor_top))
        err = sup_rel_err(self.tails.cFunc, self.truth.cFunc, self.pt)
        err_r = sup_rel_err(self.rails.cFunc, self.truth.cFunc, self.pt)
    ...
    def test_two_roles_top(self):
        ...
        print("HS two-roles top: in-solve %.3e vs eval-only %.3e "
              "(factor %.2fx, floor %.3e)"
    ...
    def test_bottom_psi_general_regime_I(self):
        ...
        reg = ce_psi_regime(
            self.hs[("tails", HS_K)].IncShkDstn[0], 2.0,
            float((1.01 * 0.98051 * (1.0 - 1.0 / 160.0)) ** 0.5
                  / (1.0 + 0.01812 / 4)))
        self.assertEqual(reg["regime"], "I")
        self.assertLess(reg["lambda_min_fiber"], 0.5)
        err = sup_rel_err(self.tails.cFunc, self.truth.cFunc, self.pb)
        err_r = sup_rel_err(self.rails.cFunc, self.truth.cFunc, self.pb)
        print("HS bottom (psi-general): tails %.3e rails %.3e (gate %.1e)"
              % (err, err_r, GATE_HS_BOTTOM))
        self.assertLess(err, GATE_HS_BOTTOM)
        self.assertGreater(err_r, 10.0 * err)
        mpc = float(cf.derivative(self.pb[0]))
        dev = abs(mpc / self.tails.MPCmax - 1.0)
        print("HS MPC at me=%.3e: dev %.2e (gate %.1e)"
    ...
    def test_two_roles_bottom(self):
        ...
        err_eval = sup_rel_err(cf_eval, self.truth.cFunc, self.pb)
        err_in = sup_rel_err(self.tails.cFunc, self.truth.cFunc, self.pb)
        print("HS two-roles bottom: in-solve %.3e vs eval-only %.3e "
    ...
    def test_ladder_monotone(self):
        ...
        print("HS ladder top: k=%d %.3e <= k=%d %.3e"
...
class TestRegimeGate(unittest.TestCase):
    def test_regime_II_warns_and_refuses(self):
        ...
        with warnings.catch_warnings(record=True) as wrec:
            ...
            gated.solve()
        hits = [w for w in wrec
                if issubclass(w.category, ConstraintEndRegimeWarning)]
...
class TestCubicPath(unittest.TestCase):
    def test_cubic_both_tails(self):
        ...
        with no x_list."""
        agent = solve_agent(CE_PARS, log_grid(1e-5, 1e4, 300), 1e-10,
                            CubicBool=True, decay_extrap_form="powerlaw",
                            decay_extrap_form_lower="kappabar")
    ...
    def test_cubic_default_untouched(self):
        a = solve_agent(CE_PARS, log_grid(1e-5, 1e4, 300), 1e-10,
                        CubicBool=True)
