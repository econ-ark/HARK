...
class testConditionCheckUnsupportedCycles(unittest.TestCase):
    ...
    length of 1. Every existing caller in the suite uses cycles=0 with
    T_cycle=1, so the early-return branch -- and the obligation to thread
    `verbose` back out through the returned tuple -- is otherwise unexercised.
    """
