...
\begin{table}[ht]
  ...
  \begin{tabular}{|>{\ttfamily}rcll|} 		
    ...
    \\ \verb|\tranShk|      & $\tranShk$   & transitory shock (combo) &  $\tranShk_{t} = \TranShkAgg_{t}\tranShkInd_{t}$
...
%    \\ \verb|\permShk|      & $\permShk$ & permanent shock (combo) & $\pmb{\permShk}_{t} = \PermShkAgg_{t} \permShkInd_{t}$
    \\ \verb|\permShk|      & $\permShk$ & permanent shock (combo) & $\pmb{\permShk}_{t} = \PermShkAgg_{t} \permShkInd_{t}$
    \\ \verb|\PermLvlAgg|  & $\PermLvlAgg$ & Permanent Productivity (level) & $\PermLvlAgg_{t+1} = \PtyGroFacAgg_{t+1}\PermLvlAgg_{t} $
    \\ \verb|\permLvlInd|  & $\permLvlInd$ & permanent productivity (idiosyncratic) & $\permLvlInd_{t+1} = \ptyGroFacInd_{t+1}\permLvlInd_{t} $
    ...
    \\ \verb|\PtyGroFacAgg|  & $\PtyGroFacAgg$ & Productivity Growth (Aggregate) & $\PermLvlAgg_{t+1}=\PermLvlAgg_{t} \PtyGro$
%    \\ \verb|\PtyGroRteAgg|  & $\PtyGroRteAgg$ & Productivity Growth Rate & $\PtyGro = (1+\ptyGro)$
    \\ \verb|\ptyGroFacInd|  & $\ptyGroFacInd$ & productivity Growth (Idiosyncratic) & $\ptyLvlInd_{t+1} = \ptyGroFacInd \ptyLvlInd_{t}$
%    \\ \verb|\ptyGroRteInd|  & $\ptyGroRteInd$ & productivity Growth Rate & $\ptyGroFac = (1+\ptyGroRte)$
    \\ \verb|\ptyGroFac|  & $\ptyGroFac$ & productivity Growth (combo) & $\ptyGroFac = \ptyGroFacInd \PtyGroFacAgg$
    \\ \verb|\ptyGroRte|  & $\ptyGroRte$ & productivity growth rate (combo) & $\ptyGroRte = \log \ptyGroFac$
    % \\ \verb|\ptyGroRte|  & $\ptyGroRte$ & productivity Growth Rate (combo) & $\ptyGroRte = \PtyGroRteAgg+\ptyGroRteInd$
    ...
    \\ \verb|\GPR| & $\GPR$ & Growth Patience rate  & $\patpGro = \pat -\ptyGroRte $
...
\\     \verb|\Mean| & $\Mean$ & \texttt{Mean\_} & The population mean of a variable & $\Mean[\permLvl]=1$
