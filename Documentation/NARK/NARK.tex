...
\usepackage{/Volumes/Sync/GitHub/llorracc/SolvingMicroDSOPs/SolvingMicroDSOPs-Latest/.resources/texmf-local/tex/latex/local-econark}
...
\begin{table}[ht]
  ...
  \begin{tabular}{|>{\ttfamily}rcll|}
    ...
    \\ \verb|\GPFac|       & $\GPFac$ & Growth Patience Factor (\texttt{Thorn}) & $\APFac = (\Rfree \DiscFac)^{1/\CRRA}/\PtyGroFac$
    \\ \verb|\GPRte|       & $\GPRte$ & Growth Patience rate (\texttt{thorn}) & $\GPRte \approx \APRte -\ptyGroRte $
    \\ \verb|\RPFac|       & $\RPFac$ & Return Patience Factor (\texttt{Thorn}) & $\GPFac = \APFac/\PtyGroFac $
