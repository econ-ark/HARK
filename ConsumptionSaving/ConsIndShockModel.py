...
class PerfForesightConsumerType(AgentType):
    ...
    def checkCondition(self,DiscFac_list,verbose):                      
        '''
        Given the preference types/discount factors, this method checks whether
        all types satisfy the growth impatiance condition (GIC), return impatiance 
        condition (RIC), weak return impatiance condition (WRIC), absolute impatiance 
        condition (AIC), finite human wealth condition (FHWC) and finite value of 
        autarky condition (FVAC). These are the conditions that are sufficient for 
        nondegenerate solutions. Depending on the model at hand, a different combination
        of these conditions must be satisfied. To check which conditions are relevant to 
        the model at hand, a reference to the relevant theoretical literature is made.
        
        Parameters
        ----------
        DiscFac_list : [np.array]
            A list which is one dimensional if there is no ex-ante heterogeneity in preferences. 
            It can either be exogenously given or endogenously generated by a module such as cstwMPC.
            
        verbose : boolean
            Specifies different levels of verbosity of feedback. When false, it only reports whether
            some types fail to satisfy a particular condition. When true, it reports all results, i.e.
            the factor values for all types and for all conditions.
        
        Returns
        -------
        feedback : in console
            The feedback of whether any types violate a sufficient condition for a nondegenerate 
            solution is printed. 
        '''        
        
        LP=self.LivPrb[0]                                               # LivPrb is defined as a list
        ...
        FVAC=np.zeros(shape=(len(DiscFac_list),1))
        exp_psi_inv=0
        exp_psi_to_one_minus_rho=0
        
        for i in range(len(self.PermShkDstn[1])):
            exp_psi_inv=exp_psi_inv+(1.0/self.PermShkCount)*(self.PermShkDstn[1][i])**(-1)    #Get expected psi inverse
        for i in range(len(self.PermShkDstn[1])):
            exp_psi_to_one_minus_rho=exp_psi_to_one_minus_rho+(1.0/self.PermShkCount)*(self.PermShkDstn[1][i])**(1-self.CRRA)  #Get expected psi to the power one minus CRRA
        j = 0
        while j < Beta_count:                                                       #Calculate the LHS of each condition  
            GIC[j]=(LP*exp_psi_inv*(self.Rfree*DiscFac_list[j])**(1/self.CRRA))/self.PermGroFac
            RIC[j]=((LP*self.Rfree*DiscFac_list[j])**(1/self.CRRA))/self.Rfree
            WRIC[j]=((self.UnempPrb**(1/self.CRRA))*(LP*self.Rfree*DiscFac_list[j])**(1/self.CRRA))/self.Rfree
            AIC[j]=(LP*self.Rfree*DiscFac_list[j])**(1/self.CRRA)
            FHWC[j]=self.PermGroFac[0]/self.Rfree
            FVAC[j]=DiscFac_list[j]*self.PermGroFac[0]**(1-self.CRRA)*exp_psi_to_one_minus_rho
            j += 1     
        
            #Check whether some types fail to satisfy some conditions
            #################################################################################
            count_GIC=0
            ...
            for i in range(len(DiscFac_list)):
                if FVAC[i] < 1:
                    count_FVAC+=1
            if count_FVAC!=len(DiscFac_list):
                fail_FVAC=len(DiscFac_list)-count_FVAC
            #################################################################################
            count_all=count_GIC+count_RIC+count_WRIC+count_AIC+count_FHWC+count_FVAC
            
            #Report the results
        if verbose==False:
            if count_GIC!=len(DiscFac_list):
                print str(fail_GIC) + ' Type(s) violates the growth impatience condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
            if count_RIC!=len(DiscFac_list):
                print str(fail_RIC) + ' Type(s) violates the return impatience condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
            if count_WRIC!=len(DiscFac_list):
                print str(fail_WRIC) + ' Type(s) violates the weak return impatience condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
            if count_AIC!=len(DiscFac_list):
                print str(fail_AIC) + ' Type(s) violates the absolute impatience condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
            if count_FHWC!=len(DiscFac_list):
                print str(fail_FHWC) + ' Type(s) violates the finite human wealth condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
            if count_FVAC!=len(DiscFac_list):
                print str(fail_FVAC) + ' Type(s) violates the finite value of autarky condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
            if count_all==len([count_GIC,count_RIC,count_WRIC,count_AIC,count_FHWC,count_FVAC])*len(DiscFac_list):
                print 'All types satisfy all conditions'

        if verbose==True:
            #################################################################################
            if count_GIC==len(DiscFac_list):
                print 'The growth impatiance factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(GIC) + ', which satisfies the growth impatiance condition.'
            else:
                print str(fail_GIC) + ' Type(s) violate the growth impatience condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
                print 'The growth impatiance factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(GIC) 
            #################################################################################            
            if count_RIC==len(DiscFac_list):
                print 'The return impatiance factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(RIC) + ', which satisfies the return impatiance condition.'
            else:  
                print str(fail_RIC) + ' Type(s) violate the return impatience condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
                print 'The return impatiance factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(RIC) 
            #################################################################################
            if count_WRIC==len(DiscFac_list):
                print 'The weak return impatiance factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(WRIC) + ', which satisfies the weak return impatiance condition.'
            else:  
                print str(fail_WRIC) + ' Type(s) violate the weak return impatience condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
                print 'The weak return impatiance factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(WRIC) 
            #################################################################################
            if count_AIC==len(DiscFac_list):
                print 'The absolute impatiance factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(AIC) + ', which satisfies the absolute impatiance condition.'
            else:  
                print str(fail_AIC) + ' Type(s) violate the absolute impatience condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
                print 'The absolute impatiance factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(AIC) 
            #################################################################################
            if count_FHWC==len(DiscFac_list):
                print 'The finite human wealth factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(FHWC) + ', which satisfies the finite human wealth condition.'
            else:  
                print str(count_FHWC) + ' Type(s) violate the finite human wealth condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
                print 'The finite human wealth factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(FHWC) 
            #################################################################################
            if count_FVAC==len(DiscFac_list):
                print 'The finite value of autarky factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(FVAC) + ', which satisfies the finite value of autarky condition.'
            else:  
                print str(count_FVAC) + ' Type(s) violate the finite value of autarky condition with the supplied parameter values. Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.'
                print 'The finite value of autarky factor value for the supplied parameter values is (a vector if there is ex-ante heterogeneity) ' + str(FVAC) 
...
class KinkedRconsumerType(IndShockConsumerType):
    ...
    def checkCondition(self,DiscFac_list,verbose):
        ...
        Given the preference types/discount factors, this method checks whether
        all types satisfy the growth impatiance condition (GIC), return impatiance 
        condition (RIC), absolute impatiance condition (AIC), weak return impatiance 
        condition (WRIC), finite human wealth condition (FHWC) and finite value of 
        autarky condition (FVAC). These are the conditions that are sufficient for 
        nondegenerate solutions. Depending on the model at hand, a different combination
        of these conditions must be satisfied. To check which conditions are relevant to 
        the model at hand, a reference to the relevant theoretical literature is made.
