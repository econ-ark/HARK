...
on GitHub (https://github.com/econ-ark/HARK), click the "Clone or download"
...
to the correct directory (see set 5), or click the green arrow "run" button in
...
- ConsumptionSavingModel/ConsPrefShockModel.py
    ...
    plicative shocks to utility.  Currently only has one model, extending
    the "kinked R" model.  When run, solves an example of the preference shock
    model with different interest rates on borrowing and saving.
...
- SolvingMicroDSOPs/StructEstimation.py
    Conducts a very simple structural estimating using the idiosyncratic shocks
...
- cstwMPC/MakeCSTWfigsForSlides.py
    Makes various figures for the slides forthe cstwMPC paper.  Requires many
    output files produced by cstwMPC.py, from various specifications, which are not
    distributed with HARK.  Has not been tested in quite some time.

Parameter and data modules:
- ConsumptionSavingModel/ConsumerParameters.py
